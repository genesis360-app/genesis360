...
| [[wiki/features/modo-basico-avanzado]] | **Modo de operación Básico vs Avanzado (WMS)** (v1.55.0, mig 207) — básico (default tenants nuevos, todos los planes): mostrador simple sin LPN/lotes/series/OC/envíos; avanzado: sistema completo, toggle DUEÑO en Config gateado a Pro+ (trial lo prueba). El modo gatea UI, nunca datos (`modoOperacion.ts` + `useModoOperacion`); kill-switch `MODO_BASICO_ENABLED`. F1 en DEV; pendiente F2 (POS/Proveedores/Config) + F3 (onboarding/planes) | 🚧 DEV |
