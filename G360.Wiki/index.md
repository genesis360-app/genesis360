...
| [[wiki/features/modo-basico-avanzado]] | **Modo de operación Básico vs Avanzado (WMS)** (v1.55.0-v1.57.0, mig 207) — básico (default tenants nuevos, todos los planes): mostrador simple sin LPN/lotes/series/OC/envíos; avanzado: sistema completo, toggle DUEÑO en Config gateado a Pro+ (trial lo prueba). El modo gatea UI, nunca datos (`modoOperacion.ts` + `useModoOperacion`); kill-switch `MODO_BASICO_ENABLED`. **F1+F2+F3 completos en DEV**. **v1.57.0:** básico "mínimo mostrador" (oculta también Recursos/Biblioteca/Facturación-cond/Sucursales-cond → 12 módulos) + **auditoría de roles** (`navVisibility.ts` puro + matriz rol×modo, 2 bugs corregidos: DEPOSITO/Recepciones, CONTADOR/Historial) + rol custom read-only en mutaciones (`permisosModulo.ts`) + e2e DEPOSITO/CONTADOR. Pendiente: deploy a PROD (mig 207 antes del merge) | 🚧 DEV |
