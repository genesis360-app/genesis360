...
- **Opinión sobre el comentario de GO (afip.js con .key/.crt):** el consejo "usar una librería" ya se cumple — usamos `@afipsdk/afip.js`. El comentario describe el modo **self-host** (cert local, firma directa a ARCA); nosotros usamos la variante **cloud** (access_token, AfipSDK firma el WSAA en sus servidores). En **Deno Edge** la firma CMS/PKCS7 local es impráctica → self-host sería proyecto dedicado (mover la EF a Node). Recomendación: quedarse en **AfipSDK cloud** para el primer cliente. Detalle + runbook en `wiki/features/facturacion-afip.md`. **Trampa de UX detectada:** el uploader de certs `.crt`/`.key` en Config (`tenant_certificates`, mig 043) es código muerto (la EF usa el token) → ocultar/relabelear.
- **▶ Próximo:** decisión de GO de deployar v1.60.0 + mig 210 a PROD (aditiva, default false = cero impacto) + operativo (CUIT activo + cert + token AfipSDK prod) + smoke real de CAE. Mientras tanto, GO puede validar el flujo end-to-end en homologación con su CUIT (sin valor fiscal, sin riesgo).
