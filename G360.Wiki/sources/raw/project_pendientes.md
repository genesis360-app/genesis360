...
> ### 🟢 ARRANCÁ ACÁ (2026-06-30 · 🎁 POS crédito auto-sugerido + 🎨 fondo de marca unificado v1.98.0 · EN PROD)
> **Estado:** **PROD = DEV = v1.98.0 (migs 001-246)** ✅ — Vercel desplegado. typecheck+build+**819 unit** verdes. Frontend-only, sin migración.
>
...
>
> **🆕 PENDIENTE GO/dev (de esta auditoría — flujo MP, hoy NO productivo):** (a) `handleVerificarPago` (SuscripcionPage) **auto-activa la suscripción desde params de URL** (`status=approved`+`preapproval_id`) **sin verificación server/webhook** → endurecer (verificar contra MP) ANTES de habilitar cobros reales; (b) **sin prorrateo** al cambiar de plan → posible doble cobro al upgradear. Ambos del lado MP.
>
...
> 1. **Hardening MP/Billing (de esta auditoría):** verificación server-side en `handleVerificarPago` (no auto-activar desde URL) + prorrateo al cambiar de plan. Va junto con el cobro MP real cuando GO conecte una cuenta de prueba.
...
> 5. **Smoke de go-live / primer cliente real** si se acerca el alta (correr `tests/specs/uat-primer-uso.plan.md` + paridad DEV/PROD antes del alta).
>
> ---
