...
> ### 🚀 ARRANCÁ ACÁ (2026-07-02 · Pricing F2/3/4 + fix cancelación + seguridad rol ADMIN + cancelar-desde-panel · v1.106.0 EN PROD · DEV = PROD)
> **Estado:** **PROD = DEV = v1.106.0** (migs 001-254, **839 unit**). Esta sesión: Pricing F2/3/4 (v1.102-103) · fix REGLA #0 cancelación de suscripción (v1.104, EF `cancel-suscripcion`) · seguridad REGLA #0 escalada rol ADMIN (v1.105, mig 254 + `invite-user` whitelist) · **cancelar suscripción desde el panel interno `admin.genesis360.pro`** (v1.106, EF `admin-api` acción `billing.cancel_subscription` + botón en `genesis360-admin/CustomerDetailPage`). **Pendiente:** (b) UAT e2e de alta MP en navegador (GO, como *Buyer Test User* en incógnito) → yo valido la cancelación por API con el token de prueba. **Nota:** repo `genesis360-admin` es separado (panel), su frontend deploya por su propio Vercel. **F2/F3 (v1.102.0):** mig 253 + 4 EFs MP + frontend. **F4 (v1.103.0):** `PricingConfigurator` en la Landing. **v1.104.0:** 🔴 fix REGLA #0 de cancelación de suscripción (EF `cancel-suscripcion` + `MiCuentaPage`/`AdminPage` propagan a MP). **RIESGO #1 RESUELTO** (GO reconfiguró los planes base de MP a $60k/$100k). **🟠 Falta:** (1) **reconciliar la fila de Fede** (`456dbf20…` sigue `active` en DB; GO ya lo canceló en MP → cancelar desde AdminPage —ahora invoca el EF— o `UPDATE tenants SET subscription_status='cancelled' WHERE id='456dbf20-355f-49af-afa1-300f50d8d3f4'`); (2) **UAT e2e de alta en el navegador** (GO, con usuarios+tarjetas de prueba de MP, titular `APRO`).
>
