...
> ### 🟢 ARRANCÁ ACÁ (2026-06-30 · 🛟 Soporte server-side + 📧 email rebrandeado + 🌐 fix Landing v1.100.0 · EN PROD)
...
> - **🧾 Facturación WSFE 100% propia (sin AfipSDK) — DECIDIDO PARA DESPUÉS (GO).** Hoy `emitir-factura` usa **AfipSDK** (`@afipsdk/afip.js`, `eb.createVoucher`, requiere `tenant.afipsdk_token`; firma WSAA "en su nube"). Verificado: NO hay integración directa al WSFE en el repo (cero `wsaa.afip`/`wsfev1`/`FECAESolicitar`/`LoginCms`); el cert del tenant se pasa a AfipSDK pero el request pasa por ellos. **GO quiere migrar a directo** ("sin terceros"). Trabajo acotado: TRA + firma CMS/PKCS#7 con el cert del tenant → WSAA `LoginCms` → TA cacheado ~12h → WSFEv1 directo (`FECAESolicitar`/`FECompUltimoAutorizado`) SOAP a AFIP → sacar AfipSDK + `afipsdk_token`. **Primero homologación** (reusar matriz A/B/C ya validada), después PROD. Beneficio: $0 terceros + datos fiscales no salen de nuestra infra. **AFIP/ARCA = $0** (no cobra por CAE); el `afipsdk_token` es **por tenant** → si cada cliente trae su cuenta AfipSDK, el costo es del cliente, no de Genesis360.
> - **📄 T&C + Política de Privacidad + consentimiento de marketing — ⏸️ DECISIÓN PENDIENTE (GO revisando, 2026-06-30).** Hoy NO existe T&C ni política de privacidad ni checkbox de aceptación en el registro (solo el disclaimer FISCAL de `FacturacionPage`). GO quiere que el cliente acepte que guardamos sus datos para marketing. **Decisión abierta que frena la implementación:** marketing como **opt-in separado opcional** (recomendado, Ley 25.326, consentimiento libre/revocable) vs **bundled requerido** (un solo checkbox T&C+marketing; más cobertura, legalmente más débil). GO va a definir cuál. Plan al implementar: página `/terminos` + `/privacidad` + checkbox(es) en `OnboardingPage` (⚠ path "confirm email ON" → el consentimiento viaja por el metadata del `signUp`, como `ob_nombre`/`ob_tipo`/`ob_pais`) + columnas DB (`terminos_aceptados_at` + `marketing_consent`, vía **migración**) + link footer Landing. **Requiere revisión de un abogado.** Ver [[reference_pricing_planes_costos]].
>
> ---
