...
> ### 🚀 ARRANCÁ ACÁ (2026-07-02 · Pricing F2/3/4 + fix cancelación + fix seguridad rol ADMIN · v1.105.0 EN PROD · DEV = PROD)
> **Estado:** **PROD = DEV = v1.105.0** (migs 001-254, **839 unit**). **v1.105.0:** 🔴 seguridad REGLA #0 — bloqueada la escalada de un DUEÑO a `rol='ADMIN'` (que da `is_admin()` → ver todos los tenants) vía `invite-user` (whitelist) + **mig 254** (trigger en `users`). El guard de `/admin` ya existía. **Pendiente:** (a) fix cancelación en el panel separado `admin.genesis360.pro` (repo `genesis360-admin`, requiere acceso); (b) UAT e2e de alta MP en navegador (GO) → yo valido la cancelación por API. **F2/F3 (v1.102.0):** mig 253 + 4 EFs MP + frontend. **F4 (v1.103.0):** `PricingConfigurator` en la Landing. **v1.104.0:** 🔴 fix REGLA #0 de cancelación de suscripción (EF `cancel-suscripcion` + `MiCuentaPage`/`AdminPage` propagan a MP). **RIESGO #1 RESUELTO** (GO reconfiguró los planes base de MP a $60k/$100k). **🟠 Falta:** (1) **reconciliar la fila de Fede** (`456dbf20…` sigue `active` en DB; GO ya lo canceló en MP → cancelar desde AdminPage —ahora invoca el EF— o `UPDATE tenants SET subscription_status='cancelled' WHERE id='456dbf20-355f-49af-afa1-300f50d8d3f4'`); (2) **UAT e2e de alta en el navegador** (GO, con usuarios+tarjetas de prueba de MP, titular `APRO`).
>
