...
| | DEV | PROD |
|---|---|---|
| APP_VERSION | `v1.59.0` ✅ | `v1.59.0` ✅ |
| Migrations | 001–**209** ✅ | 001–**209** ✅ |
| Branch | `dev` (= `main`) | `main` (release v1.59.0, PR #191) |
| Vercel | preview auto desde `dev` | PROD deploy v1.59.0 (auto desde `main`) |
| Edge Function `courier-api` | con logging + `probar` ✅ | con logging + `probar` ✅ |

**Migrations DEV pendientes de aplicar en PROD:** ninguna (208 + 209 ya en PROD). **v1.59.0 EN PROD** (PR **#191**, migs 208/209 aplicadas antes del merge, `dev=main`): recortes de modo básico (Productos→Estructura, Config→Conectividad sub-tab API) + seguridad (208: planes RLS, search_path 25→0, anon SECURITY DEFINER 29→15; 209: buckets avatares/productos con SELECT scopeado, listado cross-tenant 2→0) + npm audit react-router-dom 6.30.4 + primer e2e mutante de venta. Suite **701 unit + 158 e2e** · build verde. **Acción pendiente de GO:** activar Leaked Password Protection en Supabase Auth.
