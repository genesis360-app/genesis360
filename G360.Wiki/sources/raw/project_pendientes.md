...
> ### 🟢 ARRANCÁ ACÁ (2026-06-22 · handoff para /clear · 2da actualización post-barrido Ventas)
> **Estado:** PROD = **v1.84.0 (migs 001-240)**. DEV = v1.84.0 **+ commits test-only adelantados** (specs 53-57 + cobertura, NO en main hasta el próximo deploy; no tocan app code). Repo limpio, todo pusheado. Build/typecheck verdes. (v1.84.0 = descuento por-ítem read-only + estado "sin clave" H3 + fix label Autorizaciones + specs 50/51/52.)
>
> **▶ DÓNDE QUEDAMOS: UAT EXHAUSTIVO en curso — Ventas/POS Tanda A REGLA #0 COMPLETA.** Próxima sesión = seguir el barrido (Ventas Tanda B → módulo Caja → resto del orden).
> - **✅ Ventas/POS — Tanda A REGLA #0 CERRADA** (salvo §29 AFIP, bloqueado por GO). Specs nuevos esta sesión (todos verdes + DB-verificados + DEV limpio): **50** pagar nómina · **51** aprobación ajuste 2-actores (+🐛 fix UI label Autorizaciones `ajuste_conteo`/`bulk_edit` ya no dicen "Eliminar LPN") · **52** over-receipt bloquea · **53** crédito a favor (no supera disponible) · **54** tier mayorista · **55** venta USD · **56** guard emisor↔letra (EF directa) · **57** reserva sin seña · **FAC-27** (B ≥ umbral sin DNI → 400, vía flip RI reversible, sin spec — necesita tenant RI persistente). Inventario maestro del barrido = `tests/specs/cobertura/01_ventas_productos_facturacion.md` (reconciliado).
> - **▶ SIGUIENTE (elegir):** (A) **Ventas Tanda B** — `reserva_sena_minima_pct` (seña < N% bloquea), `reserva_penalidad_pct` (cancelar reserva retiene %→caja/crédito), `cliente_obligatorio` (3 valores) × `cliente_consumidor_final`, `reglas_canal` (tope desc/lista/requiere_cliente), `cuotas_bancos`, presupuesto vencido (L44). (B) **Módulo Caja/Bóveda** (`tests/specs/cobertura/03_caja_clientes_gastos.md`). (C) **Productos** (margen, variantes, bulk, max_productos, L49-L60).
