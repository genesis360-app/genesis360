...
>
> **▶ Tanda A e2e — EN CURSO (REGLA #0 sin e2e):**
> - ✅ **descuento SUPERVISOR sobre tope** (spec 45, **VERDE**): sesión SUPERVISOR, descuento general 30% > tope 10% → gate de clave maestra ("supera el límite del SUPERVISOR"); clave incorrecta → "Clave maestra incorrecta" (server-side); clave correcta → override autoriza y cierra el modal. Valida `validarDescuentosPorRol` (hueco $/% v1.81.0/v1.82.0) + verificación server-side de clave. Fixture `descuento_max_supervisor_pct=10` por SQL (reseteado). Corre bajo `chromium-supervisor`.
> - ⏳ **Pendientes** (priorizar): §29 fiscal runtime (AFIP homolog. — externo/flaky, en gran parte operativo de GO), límite/morosidad CC (la UI 'bloquear'; el guard server 234 ya validado 8/8 por impersonación), clave SIN configurar (Jorgito tiene clave → requiere un tenant sin clave), ajuste de inventario por rol≠DUEÑO (2 actores), conteo gate + doble conteo, over-receipt, pagar nómina (RPC `pagar_nomina_empleado`).
>
