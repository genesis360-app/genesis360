...
>
> **▶ Tanda A e2e — 3 specs VERDES (REGLA #0); resto residual documentado:**
> - ✅ **spec 45 — descuento SUPERVISOR sobre tope** (`chromium-supervisor`): descuento general 30% > tope 10% → gate de clave maestra ("supera el límite del SUPERVISOR"); clave incorrecta → "Clave maestra incorrecta" (server-side); clave correcta → override autoriza. Valida `validarDescuentosPorRol` (hueco $/% v1.81.0/v1.82.0) + verificación server-side de clave. Fixture `descuento_max_supervisor_pct=10`.
> - ✅ **spec 46 — límite de CC 'bloquear'** (`chromium`): cliente CC `limite_credito=1` + `cc_enforcement_politica='bloquear'` → venta 100% a CC → "supera el límite … Operación bloqueada.", la venta NO se crea. Capa UI del guard server `fn_ventas_cc_guard` (mig 234). Fixtures: cliente "ZZZ CC Limite Test" + policy bloquear.
> - ✅ **spec 47 — conteo por rol ≠ DUEÑO → autorización pendiente** (`chromium-supervisor`): SUPERVISOR finaliza conteo con diferencia +1 → NO ajusta al toque → "pendiente de aprobación" + fila `autorizaciones_inventario` (verificado en DB). Complementa spec 36 (DUEÑO directo). Sin fixture especial.
> - Las 3 corren verdes en **corrida combinada**; las fixture-dependientes (45/46) tienen **skip-guard** (patrón specs 35/42) → el full-suite no falla sin fixtures. Fixtures SQL reseteadas tras cada corrida (DEV limpio).
> - ⏳ **Residual (NO autoreado — alto costo de fixtures frágiles de plata/stock sobre DEV compartido, o externo; ya cubierto en capa impersonación/unit):**
>   - **§29 fiscal runtime** (AFIP homologación): externo/flaky, en gran parte operativo de GO. No conviene un e2e de CAE en suite.
>   - **morosidad CC** (deuda vencida → bloquea): requiere sembrar una venta CC vencida (fixture frágil: venta+items+vencimiento). Ya validado por impersonación en el guard 234 (8/8 incl. morosidad).
>   - **pagar nómina** (RPC `pagar_nomina_empleado`): RRHH multi-paso (empleado+nómina+caja). 
>   - **over-receipt** (recepción > OC con aprobación SUPERVISOR): OC+recepción multi-paso.
>   - **clave SIN configurar**: requiere un tenant sin clave (las sesiones e2e son Jorgito, que tiene clave).
>   - **ajuste por rol con 2 actores** (no-DUEÑO solicita → DUEÑO aprueba): spec 47 cubre el lado "solicita"; el "aprueba" necesita orquestar 2 sesiones.
>
