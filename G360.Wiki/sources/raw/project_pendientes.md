...
## Estado actual DEV / PROD — cierre sesión 2026-06-11 (3ª tanda)

| | DEV | PROD |
|---|---|---|
| APP_VERSION | `v1.53.0` | `v1.53.0` ✅ |
| Migrations | 001–**205** ✅ | 001–**205** ✅ |
| Branch | `dev` (alineado con `main`) | `main` (release v1.53.0) |
| Vercel | preview auto desde `dev` | PROD deploy v1.53.0 (auto desde `main`) |
| Edge Function `courier-api` | con logging + `probar` ✅ | con logging + `probar` ✅ |

**Migrations DEV pendientes de aplicar en PROD:** ninguna (205 ya en PROD).
...
5. 🟡 **Cheques desconectados del circuito de pago**: `cheques` sin `oc_id`/`gasto_id`; pagar con medio "Cheque" no crea el cheque; rechazado no reactiva deuda CC proveedor; depositado/cobrado no toca cuenta de origen. Feature mediana. **← próximo de la auditoría**
