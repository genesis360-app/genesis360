...
**Estado:** **PRD = v1.78.0**; **DEV adelantado** con fixes de facturación sin versionar aún (ver abajo). migs **001–224** en DEV **y PROD**. EFs (DEV+PROD): `emitir-factura` (con costo de envío), `cron-sweeps`, `admin-api` (panel de soporte). ⚠ **EF `emitir-factura` en DEV tiene 2 fixes NO deployados a PROD** (guard de tipo por emisor + normalización de alícuota) — ver punto de abajo.

**🧾 Fixes de facturación (2026-06-18, EN DEV, sin versionar) — 4 bugs, uno GRAVE:**
...
**📌 Pedido de GO (UI, para próxima sesión) — Caja Fuerte: tarjeta de "Capital total" destacada:** en `CajaPage.tsx` (pestaña Caja Fuerte / Bóveda), GO quiere que el **capital total** (`capitalTotal`, ya calculado) deje de mostrarse chiquito a la derecha de "CAPITAL DEL NEGOCIO POR CUENTA" ([CajaPage.tsx:1960-1962](../../../src/pages/CajaPage.tsx#L1960)) y pase a una **tarjeta estilo Dashboard, grande, arriba a la derecha del header** (junto/encima de los botones Ingresar/Enviar a Caja/Arquear/Extraer, ~líneas 1938-1942). **Diseño:** fondo **degradé violeta→cian**, número grande (es el dato principal de la página: "cuánta plata tengo en mi cartera/capital"). Quitar el `Total:` chico de la línea 1962. Es solo UI (no toca datos ni cálculo). **Consulta abierta de GO:** ¿finanzas/capital amerita un módulo aparte (Tesorería consolidada) o se sigue manejando distribuido (Caja + Bóveda + Cuentas de Origen + Cheques + CC + Gastos)? — evaluar; hoy el dato existe pero está disperso.

**📌 Pedido de GO (backlog) — múltiples arqueos por caja:** GO reporta que **no se puede hacer más de 1 arqueo en una caja**. A nivel datos parecería soportado (`realizarArqueo` hace INSERT en `caja_arqueos` y `arqueosSesion` es una lista; `CajaPage.tsx:~637` exige "al menos un arqueo parcial antes de cerrar") → **investigar si hay un guard/UI que bloquea el 2º arqueo** (botón que desaparece, o validación) y habilitar arqueos parciales repetidos dentro de una misma sesión de caja. Para próxima sesión.
