...
Último release: **v1.55.0** 🚧 EN DEV (**Modo de operación Básico vs Avanzado (WMS) · F1** — mig **207** `tenants.modo_operacion` (existentes → avanzado, nuevos → básico); **básico** = default en todos los planes, mostrador simple sin LPN/lotes/series/vencimientos/ubicaciones/OC/envíos/historial; **avanzado** = sistema completo, toggle DUEÑO en Config → Negocio gateado a plan **Pro+** (feature `wms`, el trial lo prueba); **el modo gatea UI, nunca datos** (ledger sigue grado WMS; productos heredados con tracking conservan flujo aun en básico); kill-switch global `MODO_BASICO_ENABLED` en brand.ts; lib `modoOperacion.ts` + hook `useModoOperacion` (+14 tests → suite unit **679**); release v1.55.0 sobre `dev`. **Pendiente: F2** (POS sin picker LPN/cotización courier · Proveedores sin OC/presupuestos/calificación · ConfigPage ocultar secciones WMS) **+ F3** (sugerencia post-onboarding por `tipo_comercio` + copy planes/landing + e2e básico) **+ aplicar mig 207 en PROD al deployar**. Detalle: `wiki/features/modo-basico-avanzado.md`).
...
| | DEV | PROD |
|---|---|---|
| APP_VERSION | `v1.55.0` 🚧 | `v1.54.0` ✅ |
| Migrations | 001–**207** ✅ | 001–**206** ✅ |
| Branch | `dev` (adelantado: v1.55.0 F1) | `main` (release v1.54.0) |
...
0. **Modo básico/avanzado — F2 + F3 + deploy a PROD** (continuación de v1.55.0, plan en `wiki/features/modo-basico-avanzado.md`): **F2** = POS sin picker LPN ni cotización courier · ProveedoresPage ocultar OC/presupuestos/calificación en básico · ConfigPage ocultar secciones WMS (ubicaciones avanzadas, estados, perfiles GS1, gates de conteo, días ABC, regla tenant, tab Envíos) · **F3** = sugerencia post-onboarding (`sugiereModoAvanzado` ya en la lib) + copy de planes/landing + e2e smoke del modo básico. Al deployar: mig 207 en PROD antes del merge.
