...
| | DEV | PROD |
|---|---|---|
| APP_VERSION | `v1.46.0` | `v1.46.0` |
| Migrations | 001–**198** ✅ | 001–**198** ✅ |
| Branch | `dev` (alineado con `main`) | `main` (release v1.46.0) |
| Vercel | preview auto desde `dev` | PROD deploy v1.46.0 |

**Migrations DEV pendientes de aplicar en PROD:** ninguna (198 ya en PROD).
...
- **RH4 — Frecuencia + anticipos (B1/B10) — PENDIENTE:** frecuencia de liquidación configurable por empleado · anticipo simple con descuento automático en la próxima liquidación.
- **RH5 — Vacaciones 2.0 (C1-C7) — PENDIENTE:** días por antigüedad LCT (sugerencia)+override · aprobación configurable por rol · plazo de aviso · alerta de solapamiento + calendario · partidas configurables · remanente auto+límite · pago dentro del sueldo.
...
**Estado:** RH1+RH2+RH3+RH6 ✅ en PROD. **Faltan RH4, RH5, RH7, RH8** + los diferidos dentro de RH6 (QR público de fichado, auto-descuento tardanza en nómina). Libs `rrhhNomina.ts` + `rrhhAsistencia.ts` + `reciboSueldoPDF.ts`.
