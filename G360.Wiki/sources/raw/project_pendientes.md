...
**Estado:** **PRD = DEV = v1.72.0**, migs 001–214, EF `emitir-factura` DEV+PROD (con `cae` + `CbtesAsoc`; sin cambios en v1.72.0). Sesión 2026-06-15/16: v1.66→v1.71 (UX + auditoría básico + costuras + click-through GO) y **v1.72.0** (NC fiscal PDF/imprimir/email · rol Lector · roles custom→Pro · fix sucursal reingreso · fix NC tipo AFIP 10040 · auto-A/B/C Exento · 3 guards fiscales).
...
7. 🟠 **Sweeps lazy sin cron** (riesgo creciente): intereses CC, reservas vencidas, servicios recurrentes solo corren al abrir la página correcta. **La infra de cron externo YA existe** (GH Actions + secrets `SUPABASE_URL`/`SUPABASE_ANON_KEY`, patrón `birthday-notifications.yml`) — agregar workflows análogos por sweep es barato. **← próximo candidato de la auditoría**
...
10. 🟠 **ISS — Sucursales en modo BÁSICO (decisión de arquitectura).** Síntoma (GO, Kiosko Buildi básico, 2026-06-15): tras una devolución el stock reingresado **solo se veía en "Todas las sucursales"** (la línea quedaba con `sucursal_id` NULL) y aparecía **una línea por unidad** (se ve feo). **Causa raíz parcial YA arreglada** (v1.72.0-DEV): el reingreso de Devolver/Anular omitía `sucursal_id` → ahora hereda el de la venta (`inventario_lineas` + `movimientos_stock`, 4 spots en `VentasPage`). **Decisión de fondo pendiente — dos caminos:** **(A)** bypassear toda la lógica de sucursal en básico (sacar filtros de sucursal de todas las páginas) — más simple de UI pero rompe la consistencia de datos si el tenant crece a multi-sucursal/avanzado; **(B, RECOMENDADO)** *sucursal default oculta*: el básico siempre tiene 1 sucursal, **se fija como contexto activo** (nunca "Todas"/NULL), el selector se oculta y **todo escribe/lee esa sucursal**. Bajo riesgo + el pase a avanzado/multi-sucursal "just works" (los datos ya están en la default). Falta: forzar `sucursalId = la única` en básico (hoy puede quedar en "Todas") + auditar que ningún otro write omita `sucursal_id`. **Sub-issue UX — proliferación de líneas en básico:** el Inventario muestra una línea WMS por ingreso ("Ingreso DD/MM — 1u"); para básico conviene **consolidar el reingreso en la línea existente del producto** (misma sucursal, sin ubicación/estado/lote) en vez de crear líneas nuevas, y/o mostrar el **origen/motivo** (la columna `inventario_lineas.notas` YA guarda "Devolución de venta #X" / "Anulación de venta #X" → exponerla es gratis). Idealmente el básico muestra **agregado por producto**, no el detalle línea-a-línea (eso es WMS). Ver [[reference_basico_stock_null_ubicacion_estado]].
