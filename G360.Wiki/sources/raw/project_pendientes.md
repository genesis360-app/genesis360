...
> - **✅ Caja/Bóveda — spec 64 (cierre con diferencia):** sobrante $100 → ajuste `ingreso` $100 en `caja_movimientos` + `diferencia_cierre=100` (DB-verificado). Env-gated `E2E_CAJA_CIERRE_DIF=1` (abre/cierra caja real + email). **▶ Próximo incremento de Caja:** cierre **ajeno** con clave maestra (CON/SIN), **extracción de Bóveda** (egreso real), `diferencia_caja_umbral` (alerta), doble validación B7. Reconciliación: G1/G2/G3 de `cobertura/03` ya parcialmente cerrados por migs 234 + specs 40/41/45/46/48/49.
