...
- 🔒 **Mig 427 — ML/TN: cola de sincronización y vínculos solo desde el servidor.** `integration_job_queue` pasa a
  ...
  edita Configuración; `anon` sin nada. "Forzar sync de stock" pasa a la RPC server-side `fn_forzar_sync_stock`.
  🛑 Gotcha evitado en la misma migración: `fn_enqueue_sync_precio` (trigger de `productos`) no era `SECURITY
  DEFINER` — con el REVOKE de INSERT directo, cambiar el precio de un producto vinculado habría fallado. Antes: un
  CAJERO por REST podía encolar cualquier job o reescribir un vínculo, y el worker mandaba el stock/precio de un
  producto a otra publicación de la misma cuenta (hallazgo REGLA #0 arrastrado de la mig 424/cont. 69). Ver
...
typecheck+build+suite de tests verdes, commiteado junto con el wiki en un solo commit `28d9291e`, tag+release
`v1.174.0` publicados. **2 migraciones nuevas: 373 y 374.**
...
`56f48fe8`), tag+release `v1.172.0` publicados sobre `dev`. **1 migración nueva: 371.**

- **Caja en Dólares (relevamiento G5) — Fase 3 (ciclo operativo) completa**: el ciclo
  apertura→movimientos→arqueo→cierre de una Caja USD ya funciona de verdad — fix del bug de formato ("$"
  en vez de "US$", `formatMonedaCaja`), conteo por denominación de billete USD (`ConteoDenominaciones.tsx`,
  solo enteros), umbral de diferencia propio en USD, y 2 triggers server-side (defensa en profundidad,
  `fn_validar_traspaso_misma_moneda`/`fn_validar_rol_opera_caja_usd`) que bloquean traspaso entre cajas de
  distinta moneda y abrir una Caja USD sin permiso de rol. Bóveda excluye Cajas USD como origen/destino
  (hallazgo de code-review, cerrado en la misma sesión).
...
`310d9b3b` + bump de versión `0b4d431a`, tag+release `v1.171.0` publicados sobre `dev` (no sobre `main`).
**11 migraciones nuevas: 360 a 370.**
...
- **Caja en Dólares (relevamiento G5) — Fase 1 (cimientos) + Fase 2 (permisos/config) completas** (migs
  ...
  propios en USD (arqueo, clave maestra); checkbox `productos.acepta_cualquier_moneda`. **Fase 3 (ciclo
  operativo) quedó commiteada por separado — ver v1.172.0 arriba.**
...
## v1.166.0 — 🔍🛑 Trazabilidad completa de una venta en `/historial` (mig 351) + fix REGLA #0: el stock reservado quedaba atascado tras entregar el pedido — ✅ EN PROD (2026-08-11)

Continuación directa de la sesión de v1.165.0/v1.165.1, mismo día. Construye lo que había quedado
anotado como pendiente y explícitamente diferido en esa sesión ("trazabilidad full de venta") —
arrancando por venta, no por todo el inventario ("es mucho hacerlo todo junto" — GO).
...
155) desde `logActividad()`. `LogParams` gana `venta_id?: string | null`; se suma `'envio'` al tipo
`EntidadLog` (Envíos no tenía tipo de entidad propio en el log).

**Auditoría real de gaps ANTES de diseñar** (mismo criterio: "empezar por auditar TODOS los puntos
donde Pedidos/Envíos deberían loguear y no lo hacen"): `EnviosPage.tsx` **tenía CERO llamadas a
`logActividad()`** en todo el archivo (146KB, uno de los módulos más grandes) — se agregó crear envío,
cambio de estado y eliminar envío (a propósito quedaron afuera ediciones de campos sueltos como
combustible/domicilio/POD/tracking automático — detalles logísticos internos, no hitos del ciclo de
vida). `PedidosPage.tsx` tenía la mayoría de las transiciones logueadas, pero `fn_pedido_cerrar` y
`fn_pedido_deslanzar` no logueaban nada — completadas.
...
actividad de esa venta sin paginar y reutiliza el cruce con `venta_item_despachos` para mostrar de qué
LPN/ubicación salió cada línea. Distingue "no existe esa venta" de "existe pero es de antes de que
existiera esta trazabilidad" (sin backfill retroactivo). Verificado end-to-end contra DEV real: insert
de prueba en la venta #622 → visible en `/historial` filtrando por "622" → borrado; una venta vieja
(anterior a la mig 351) mostró el mensaje correcto de "sin trazabilidad retroactiva".

**2. 🛑 Fix REGLA #0 — bug real encontrado MIENTRAS se construía la trazabilidad.** GO preguntó "¿el
picking realmente rebaja stock?" viendo el mensaje del guard de v1.165.0. Verificado contra el SQL
real (no contra el comentario/supuesto): **el picking NUNCA rebaja stock ni toca `cantidad_reservada`**
para un pedido nacido de una venta (`fn_completar_tarea_picking`, `fn_generar_tareas_picking_pedido_venta`,
`fn_pedido_entregar_retiro` — ninguna de las tres toca `inventario_lineas.cantidad`,
`cantidad_reservada` ni `productos.stock_actual`; documentadas con comentarios explícitos desde las
migs 316/320/323). El único código que rebaja de verdad es el botón "Finalizar (rebaja stock)" de
`VentasPage.tsx`.
...
Resultado: en el flujo venta reservada → Pedido → Picking → retiro en mostrador, una vez que el pedido
se entregaba, el stock que la venta había reservado quedaba reservado **para siempre**, sin ningún
camino de código para convertirse en un rebaje real.

Fix: la query pasa a `.not('estado', 'in', '(cancelado,entregado)')` — el botón "Finalizar" reaparece
una vez que el pedido está `entregado`. Los 2 banners de aviso (ficha de venta + modal de pago MP) se
corrigieron para no decir "el rebaje se hace desde Pedidos → Picking" (falso) sino explicar que la
mercadería se está preparando y hay que volver una vez entregada.

**Verificado con datos reales de DEV, no solo lógica**: venta #599 (reservada) con su pedido #91 ya
`entregado` — query REST directa (JWT real, respetando RLS) confirmó que la query vieja devolvía el
pedido #91 (bloqueaba el botón) y la nueva devuelve vacío (lo desbloquea). Esa venta sigue en ese
estado en DEV, dejada como evidencia viva del bug — sin backfill/fix retroactivo de datos (misma regla
de no reescribir históricos). **No confundir con la venta #619** (caso de estudio de v1.165.0, abajo)
— son ventas reales distintas del mismo tenant.

**Alcance a propósito acotado a VENTA** — la idea original de GO también mencionaba trazabilidad de
"inventario" en general; queda parcialmente cubierta vía el cruce con `venta_item_despachos` que ya
mostraba LPN/ubicación, pero esto no es un rediseño completo de trazabilidad de inventario, solo de
venta.

**Deploy**: `APP_VERSION` v1.166.0, commit `e14179b5` en `origin/dev`. **PR #326 mergeado** (merge
commit `95e837f6`), **tag + GitHub release `v1.166.0`** publicados. Migración 351 aplicada y
verificada en DEV y PROD antes del merge. Bundle real de Vercel PROD verificado con la cadena
"v1.166.0".

Ver `log.md` (2026-08-11), `sources/raw/project_pendientes.md` ("ARRANCÁ ACÁ"),
`wiki/database/migraciones.md` (mig 351), [[wiki/features/reportes-metricas]] "Trazabilidad completa
de una venta", [[wiki/features/pedidos]] "Cuarta barrera" (corrección) y "Links relacionados",
[[wiki/features/ventas-pos]], [[wiki/features/envios]].

---

## v1.165.0 — 🛑 REGLA #0 (inventario): rebaje de stock por un solo camino (venta #619) + fix Compras (`registrar_pago_oc`) + 2 UX (migs 349+350) — ✅ EN PROD (2026-08-11), corregido de fondo en v1.166.0 (ver arriba)
...
**`APP_VERSION` v1.165.0, commit `c3ea45aa`. PR #324 (`dev`→`main`) mergeado, tag+release `v1.165.0`
confirmados en PROD.** Migraciones 349 y 350 aplicadas y verificadas en DEV y PROD antes del merge (DDL
aditivo/no-destructivo primero, mismo criterio de siempre). **v1.165.1 (PR #325, mismo día):** patch de
UI — fix del footer de conteo (`ListaConteoFooter` pintado por `AppLayout` en vez de `sticky` dentro de
la lista, corrige que quedara flotando encima de la última fila), sin migración propia — también en
PROD.

Pendiente anotado en esta sesión, **construido en la sesión siguiente (v1.166.0, ver arriba)**: GO
pidió trazabilidad completa de una venta en `/historial` (filtrar por número de venta, ver
pedidos/envíos/devoluciones/movimientos de stock asociados) — quedó como diseño propuesto y
explícitamente diferido en esta sesión ("hacerlo en estos días, apenas terminemos con lo que estamos
haciendo"). Esa misma verificación de "cómo rebaja el picking" para construir la trazabilidad encontró
además un bug real de REGLA #0 en el guard de arriba (`pedidoActivoVenta` no excluía `'entregado'`) —
ver [[wiki/features/reportes-metricas]] y el bloque v1.166.0 arriba.
...
MercadoLibre/TiendaNube (Fase 1.5 del roadmap de integraciones — ver
...
(`jjffnbrdjchquexdfgwq`)** — dos mecanismos independientes: (1) **ajuste automático por margen
objetivo**, opt-in por producto (`productos.reajuste_margen_auto`), toca `precio_venta` (precio base,
único cross-canal) vía el sweep `fn_evaluar_repricing_margen` (`service_role`, Edge Function
`repricing-sweep` con cron cada 6h), configurable por tenant (automático / alerta / automático desde
un monto, con tope de suba y umbral de aviso); genera autorizaciones en la pantalla YA EXISTENTE
(nuevo tipo `'repricing_margen'`) en vez de aplicar directo si el modo no es automático; (2) **ajuste
% propio por canal** (`productos.precio_ajuste_meli_pct`/`precio_ajuste_tn_pct`, independiente del
mecanismo 1): el precio PUBLICADO en cada canal se deriva del precio base + ese %, sin tocar
`precio_venta`.

**🔴 Hallazgo real del `migration-reviewer` antes de aplicar, corregido**: el trigger que dispara el
push de precio a MELI/TN reaccionaba a CUALQUIER cambio de precio sin gate — como el checkbox "Sync
precio" del mapeo de productos viene activado por default desde hace tiempo (mig 065, "dormido"), esto
habría despertado el push automático para cualquier producto ya mapeado sin participar de D3
(verificado con datos reales: afectaría 2 ítems de una cuenta MercadoLibre REAL conectada en DEV).
Corregido con un gate que solo dispara para productos que efectivamente optaron a D3.

**📦 Edge Functions `meli-stock-worker` (aplica el % de ajuste MELI al publicar), `tn-stock-worker`
(no soportaba precio en absoluto — se le agregó desde cero) y `repricing-sweep` (nueva)** deployadas
en DEV y PROD. **UI**: card "Repricing automático por margen" en Config → Integraciones, toggle +
2 campos de % de ajuste por canal en la ficha de producto, y nueva rama en el tab Autorizaciones de
...
## v1.161.0 — 🧩📦 Armado automático de kits para combos TN/MELI (D2) EN PROD (mig 345 + webhooks tn-webhook/meli-webhook) — falta la prueba end-to-end con una orden real — ✅ **PROD** (2026-08-08)

Deploy completo a PROD del backend de "Combos automáticos TN/MELI" (Fase D2 del roadmap de
integraciones), construido y verificado por RPC en la sesión anterior — **100% CERRADO**: **PR #319**
(`dev`→`main`) mergeado limpio, sin conflictos (merge commit
...
**🧩 Migración `345_armado_kits_automatico_d2.sql` aplicada en DEV Y PROD** — `wms_tareas.tipo` suma
`'armado'` y `wms_tareas.origen` suma `'marketplace'`; RPCs nuevas `fn_iniciar_armado_kit_auto` (sin
`auth.uid()`, invocable con `service_role` desde un webhook, todo-o-nada con filtro de canal
`disponible_tn`/`disponible_meli` aplicado por primera vez a una reserva ENTRANTE) y
`fn_completar_tarea_armado`; `fn_cancelar_tarea_wms` extendida para liberar la reserva de un armado.
Confirmado en PROD con `list_migrations` + queries directas (CHECK y funciones presentes en ambos
ambientes). Detalle completo de las RPCs y su verificación por SQL directo: `wiki/database/migraciones.md`
(mig 345), [[wiki/features/wms]].

**📦 Edge Functions `tn-webhook`/`meli-webhook` deployadas en DEV (v21/v25) y PROD (v20/v13)** vía
`deploy_edge_function`, con el wiring del armado automático: si tras la reserva FIFO normal sigue
faltando cantidad y el producto es un kit, invocan `fn_iniciar_armado_kit_auto`, best-effort (nunca
bloquea la venta). Sanity check post-deploy contra DEV confirmó que el deploy no rompió el boot de
ninguna de las dos funciones (respuestas esperadas ante payload vacío/topic de test, sin crashear).

**🛑 Pendiente real, no bloqueante del deploy en sí: la prueba end-to-end con una orden real.** El
armado automático nunca se disparó contra un webhook real de TN/MELI — falta crear un producto kit
...
store `7610321`) y generar una orden real ahí — requiere a GO o Fede (Claude Code no tiene ese acceso).
La lógica de las RPCs ya está 100% verificada con datos de prueba reales (todo-o-nada, reserva, filtro
de canal, completar, cancelar).

Detalle completo: `wiki/database/migraciones.md` (mig 345), [[wiki/integrations/tienda-nube]],
[[wiki/integrations/mercado-libre]], [[wiki/integrations/roadmap-apis]], [[wiki/features/wms]].
...
importador de productos NUNCA funcionó** — el payload de insert/update siempre mandaba `notas`,
columna que ninguna migración había creado; PostgREST rechazaba el request completo (`PGRST204`) y
el código nunca revisaba `error`, así que reportaba "X creados" con la tabla en CERO filas nuevas.
Fix: mig 288 agrega `productos.notas` + manejo real de errores en el importador (`erroresDetalle`)
...
siempre en unidades base (stock/rebaje/margen sin cambios). Fix de un bug real de agrupamiento de
combos con multi-UoM del mismo producto (`producto_id + unidad_medida_id`) + UoM visible en
...
OR) + columna de solo lectura "Estructura" en `InventarioPage`. Bug real cazado por el e2e 100 (el
