...
auditoría de seguridad del 2026-09-20 — `marketplace-api`, `data-api` y `transportista-subir-archivo`
llevaban la cuenta en un `Map` en memoria del isolate de Deno, que se pierde en cada cold start y no es
compartido entre los isolates en paralelo que corre Supabase de la misma función. Tabla nueva
`rate_limit_contadores` (sin `tenant_id` a propósito) + `fn_rate_limit_consumir` (atómica, `SECURITY
DEFINER`, `EXECUTE` solo `service_role`) + cron horario de limpieza. De paso se cerraron 2 hallazgos
nuevos: el límite se esquivaba falseando `x-forwarded-for`, y en `data-api` probar API keys al azar no
chocaba con ningún tope. Verificado en DEV con tráfico real (60/70 pasan en `marketplace-api`, 20×401+5×429
en `data-api`, 30×400+5×429 en `transportista-subir-archivo`) y **en PROD con tráfico real**: 70 requests
a `marketplace-api` → **64×403 + 6×429**, reconciliando exacto contra la tabla (ventana de las 02:15 cerró
con contador 66). `schema_full.sql`: **171 tablas** (era 170), policies siguen en 234 (la tabla es
deny-all, no agrega ninguna). De paso este deploy limpió 3 drifts de Edge Functions más (`marketplace-api`
prod 21→0, `data-api`/`transportista-subir-archivo` 34/25→0). Detalle en
[[wiki/architecture/guards-server-side]] ("Tanda G — backlog") y `log.md` (2026-09-22, `deploy`).

🛡️ **Migraciones 430 y 431 — ✅ EN DEV Y EN PROD** (deploy `v1.229.0`, 2026-09-22), las dos de la auditoría de
seguridad completa del 2026-09-20.

- **430** (`storage_aislamiento_por_negocio_y_search_path`): cierra la fuga real de lectura entre negocios en el
  bucket `archivos-biblioteca` (policies SIEMPRE TRUE) + `productos` (INSERT/UPDATE sin filtrar tenant) +
  `search_path` de `fn_enqueue_tn_fulfillment_sync`.
- **431** (`otp_entrega_y_vencimiento_link_cuenta`), 2ª tanda: el OTP de entrega (`envio_otp`) pasa de `random()`
  de Postgres (no criptográfico) a `extensions.gen_random_bytes`, con **máximo 5 intentos** y **un solo código
  activo por envío** (antes `verificar_otp_envio`, con GRANT a `anon`, no tenía ningún freno) · el link público
  de estado de cuenta del cliente (`clientes.cuenta_token`) deja de ser eterno: `cuenta_token_creado_at` +
  `tenants.cuenta_token_dias` (default 90, 0 = no vence) + trigger `clientes_cuenta_token_fechado` para que
  fecharlo no dependa de que el frontend se acuerde.

No es la única corrección de esa auditoría: otros hallazgos (guard `CRON_SECRET` en 15 sweeps, validación real de
`modo-webhook`/`tn-webhook`/`meli-webhook`, auth en `scan-product`/`scan-ticket`, XSS en impresión de QR,
política de contraseñas, interruptor `MP_WEBHOOK_SIG_ENFORCE`, fix de `mp-ipn`, SSL forzado) son código de Edge
Functions/frontend/Auth, sin migración de DB propia. Detalle completo en
[[wiki/architecture/guards-server-side]] ("Tanda G" y "Segunda tanda") y `log.md` (2026-09-20 ×2, 2026-09-22).
...
llevaron a PROD en un **segundo deploy** autorizado por GO: **PROD = `v1.227.0`, migs 001-429** (399 filas en
`schema_migrations`, última `429_ayuda_cursos_y_recursos`). Policies DEV = PROD: `public` **234**
(`3c7d0c745f57e8d161dea36b4a354bc0`), `storage` 40 (`fd729ff17b258d8bcdda0481c1e45135`), `cron` 2
(`99253f467c0e3919dad8823048ac1b94`). Verificación de construcción en DEV: tsc + build + eslint verdes, **1848 unit
tests** (112 archivos), `schema_full.sql` regenerado (170 tablas, 239 funciones, **234 policies**), e2e nuevos
**153** (ML/TN, mutante) y **154** (Cursos y recursos), UAT **§63/§64/§65** + 60.8 actualizado. Verificación del
deploy a PROD: `fn_enqueue_sync_precio`/`fn_forzar_sync_stock`/`fn_registrar_pago_manual` con el mismo `md5` que
DEV, grants por tabla confirmados, PR **#351** (merge `0b65bc45` en `main`), Edge Functions `admin-api` y
`ai-assistant` redeployadas en PROD (diff 0 con `scripts/auditar-edge-functions.sh`), smoke PostgREST (anon key)
...
(seed de tenant, avisos CC/OC, las 8 de precio programado + `vw_tareas_repositor`, `fn_notificar_sync_precio_fallido`,
las 8 de soporte) — confirma también los acentos. Permisos verificados en PROD: `tareas_repositor` sin
INSERT/DELETE para `authenticated` y UPDATE solo en 6 columnas; `precios_programados` sin nada para `anon`; RPC de
soporte solo `authenticated`; `support_tickets`/`support_messages` sin grants para `anon`/`authenticated`. Mig 420:
0 motivos viejos activos, 16 nuevos (8 negocios × 2). Crons activos: `aplicar-precios-programados` corrió 15 veces
en 15 minutos sin fallos; `notif-precios-programados-manana` y `notif-cc-vencidas` activos. El único ticket de PROD
("Aviso de pago manual") quedó con `usuario_id` por el backfill de la 426. PROD pasó a **396 filas en
`schema_migrations`**. Paridad de policies por schema DEV=PROD: `public` 231 (`d8325817…`), `storage` 40
(`fd729ff1…`), `cron` 2 (`99253f46…`). Smoke de PostgREST (anon key): `precios_programados`/`tareas_repositor`/
`support_tickets` 401, RPC `fn_soporte_mis_consultas` 401, RPC inventada 404, columna inventada 400. PR app **#350**
(merge `1d8e477d`) y panel `genesis360-admin` **#5** (merge `e647dd6`); releases `v1.222.0`-`v1.225.0` promovidos de
prerelease a release, `v1.226.0` **Latest**. Edge Functions redeployadas: `admin-api` (v12),
`billing-manual-avisar-pago` (v2, después de la 426), `marketplace-webhook` (v20) y `ai-assistant` (v11). 🐛
**Gotcha nuevo**: el deploy de una EF por CLI **conserva** el `verify_jwt` que ya tenía la función —
`marketplace-webhook` quedó en `false`; corregido con un `PATCH` a la Management API
(`/v1/projects/{ref}/functions/marketplace-webhook {"verify_jwt": true}`) — ver [[wiki/development/deploy]].
`bash scripts/auditar-edge-functions.sh` sobre las 4 redeployadas: diff 0 en PROD y DEV (`marketplace-webhook`
sigue sin existir en DEV, a propósito). Pre-chequeo: Kalken (primer cliente real) no estaba usando la app al
momento del deploy (último login/refresh 2026-09-14 23:41 UTC, última venta 21:03 UTC).

**🗂️ Migración 426 — ✅ EN DEV Y EN PROD** (2026-09-15). Revisada por `migration-reviewer`: APTA, con sus 2 sugerencias aplicadas antes de aplicarla (lock en los topes anti-spam, chequeo de usuario activo en las RPC de lectura). Verificada en DEV: las 4 RPC (`fn_soporte_crear_consulta`/`fn_soporte_responder`/`fn_soporte_mis_consultas`/`fn_soporte_consulta`) SECURITY DEFINER + `search_path`, `EXECUTE` solo para `authenticated` (sin `anon`); `support_tickets`/`support_messages` siguen sin privilegios de tabla (heredado de la 425); acentos correctos. SQL con los triggers reales, impersonando al cajero: nota interna → 0 avisos y no aparece en su hilo; respuesta del equipo → aviso con `action_url` a la consulta; consulta resuelta que el cliente reabre al escribir; cerrada rechaza responder. Bucket `soporte-adjuntos` + 2 policies de storage verificadas (propio, o DUEÑO/SUPER_USUARIO del negocio). Aplicada en PROD antes de redesplegar `billing-manual-avisar-pago` (que inserta columnas nuevas). Ver [[wiki/support/plataforma-soporte]].

**🗂️ Migración 425 — ✅ EN DEV Y EN PROD** (2026-09-15). Verificada en DEV con el trigger real dentro de un bloque que se revierte: ticket del cliente → 2 avisos (el largo recortado a 498), ticket interno → 0, autor de otro negocio → 0; sin la migración daba 0 (mutante). SECURITY DEFINER + `search_path`, sin `EXECUTE` para `anon`/`authenticated`, acentos correctos; `support_tickets`/`support_messages` quedan solo para `postgres` y `service_role` (PostgREST con anon → 401). Sin dependencia con el frontend de la app; el cambio del panel (`genesis360-admin`, commit `5ce8582`) va junto con el resto en PR #5.

**🗂️ Migraciones 423-424 — ✅ EN DEV Y EN PROD** (2026-09-15). `migration-reviewer`: APTAS (se aplicó su recomendación antes de aplicarlas). 423 verificada: las 7 funciones con SECURITY DEFINER + `search_path`, `EXECUTE` para `authenticated` solo en programar y cancelar, `tareas_repositor` sin INSERT/UPDATE/DELETE de tabla para `authenticated` y UPDATE en 6 columnas, vista con `security_invoker` y las 3 columnas nuevas al final, trigger del guard, acentos (`DUEÑO` = `c391`) y el cron activo. 424 verificada: SECURITY DEFINER, sin `EXECUTE` para `anon`/`authenticated`, trigger con su `WHEN`. Smoke de PostgREST con control negativo (columna inventada → 400). **Orden en el deploy:** después de la 422 y antes del frontend. Sin la 423, Alertas no muestra las etiquetas vencidas y Config no guarda la anticipación, pero nada se rompe.

**🗂️ Migraciones 421-422 — ✅ EN DEV Y EN PROD** (2026-09-14, aplicadas a PROD el 2026-09-15). 421 verificada: las dos consultas de destinatarios usan DUEÑO/SUPER_USUARIO, SECURITY DEFINER + `search_path`. 422 verificada: tabla con RLS (solo SELECT del negocio, `anon` sin privilegios), `fn_programar_precio` y `fn_cancelar_precio_programado` ejecutables solo por `authenticated`, `fn_aplicar_precios_programados` y el aviso del día anterior solo por el cron, los dos jobs activos y acentos correctos.

**🗂️ Migración 420 — ✅ EN DEV Y EN PROD** (2026-09-14, aplicada a PROD el 2026-09-15). Verificada en DEV: bytes UTF-8 de "dueño", `SECURITY DEFINER` + `search_path=public`, 7 negocios con los motivos nuevos y los dos viejos desactivados, motivos propios intactos. En PROD: 0 motivos viejos activos, 16 nuevos (8 negocios × 2) — "Extracción / Retiro" y "Gastos varios" desactivados en todos los negocios, Kalken incluido.
...
| # | Archivo | Qué hace |
|---|---|---|
| 433 | `433_desactivar_usuario_corta_acceso.sql` | 🔐 **"Desactivar" un usuario le corta el acceso de verdad — ⚠️ ESCRITA, SIN APLICAR (ni DEV ni PROD)** (2026-09-24, commit `34053f00`; salió contestando una pregunta de GO sobre las cuentas de los empleados). El agujero: `get_user_tenant_id()` —gobierna el `USING` de casi todas las policies de `public`— no miraba `users.activo`; tampoco `users_select`, ni `loadUserData`, ni ningún guard del frontend. Y "Desactivar" es la única acción que existe sobre un usuario (no hay eliminar): desde la app no había forma de cortarle el acceso a un empleado que se fue. (1) `get_user_tenant_id()` e `is_admin()` agregan `AND coalesce(activo, true)` (`users.activo` es NULLABLE — `AND activo` a secas habría dejado afuera cualquier fila con NULL, mismo criterio que `fn_soporte_ticket_detalle`); (2) trigger `trg_guard_baja_usuario` (`BEFORE UPDATE OF activo`) bloquea que alguien se dé de baja a sí mismo o al último DUEÑO activo del negocio; (3) `fn_estado_usuario_actual()` (SECURITY DEFINER, solo sobre uno mismo) para que la app explique qué pasó. 🛑 **El frontend era la mitad imprescindible**: sin `fn_estado_usuario_actual()`, un usuario dado de baja tampoco puede leer su propia fila, así que `loadUserData` lo confundiría con "no tiene negocio" y `AuthGuard` lo mandaría a crear un negocio nuevo con su misma identidad (chocaría contra su fila vieja con un error crudo de SQL) — lo marcó el `migration-reviewer` como el hallazgo más importante. Ahora `AuthGuard` muestra "Tu acceso fue dado de baja" antes del redirect a onboarding. Confirmado que `admin.genesis360.pro` (autentica contra `support_agents` con `service_role`) y el alta de negocio (triggers de seed usan `NEW.id`) **no se ven afectados**. Revisión del `migration-reviewer`: APTA para DEV. 🔴 **El conector de Supabase se desconectó a mitad de sesión**: quedó escrita y commiteada pero **sin aplicar en DEV ni en PROD** — no debe ir a PROD sin antes aplicarla y probarla a mano en DEV (Kalken tiene empleados reales). UAT §67: 7 escenarios, 4 en rojo por falta de esa prueba. Ver [[wiki/features/autenticacion-onboarding]] ("Desactivar corta el acceso de verdad"), [[wiki/architecture/multi-tenant-rls]] y [[wiki/database/rls-policies]]. |
| 432 | `432_rate_limit_persistente.sql` | 🔒⏱️ **Rate limiting persistente para las Edge Functions públicas** (cierra el pendiente 3 del backlog de la auditoría de seguridad del 2026-09-20, commit `2b585f31`). `marketplace-api`/`data-api`/`transportista-subir-archivo` contaban en un `Map` en memoria del isolate de Deno — se pierde en cada cold start y no se comparte entre los isolates en paralelo que corre Supabase de la misma función, así que el límite efectivo era 60 × (cantidad de isolates), un número que no se controla ni se conoce. Tabla `rate_limit_contadores` (bucket, identidad, ventana_inicio, contador; **sin `tenant_id` a propósito** — es infraestructura del borde) + `fn_rate_limit_consumir` (`SECURITY DEFINER` + `search_path`, incrementa atómicamente con `INSERT … ON CONFLICT DO UPDATE … RETURNING`, `EXECUTE` solo `service_role`) + `REVOKE ALL` a `anon`/`authenticated` + RLS sin policies (deny-all) + cron horario `cleanup_rate_limit_contadores`. 🩸 **Gotcha del `migration-reviewer`**: el `DELETE` del cron compara contra `ventana_inicio` (cuándo la ventana ABRIÓ, no cuándo cerró) — el margen (2 horas) tiene que ser MAYOR que la ventana más larga aceptada (`p_ventana_seg` tope 3600); si se sube el tope hay que subir el margen en el mismo commit, o el cleanup borra contadores de ventanas todavía abiertas y el límite se reinicia solo. Módulo compartido nuevo `supabase/functions/_shared/rateLimit.ts` (primer `_shared` del repo): mantiene el `Map` local como piso y hace fail-open contra la base a propósito (un hipo de la base no tira la API pública entera). De paso corrige 2 hallazgos nuevos: el límite se esquivaba falseando `x-forwarded-for` (ahora prioriza `cf-connecting-ip`, no falseable) y en `data-api` probar API keys al azar era gratis (nuevo cubo de intentos fallidos por IP). Verificado en DEV con tráfico real: 70 requests a `marketplace-api` (10 en paralelo) → exactamente 60 pasan/10 dan 429 con UNA fila de contador 70 (atomicidad); `data-api` con 25 keys inventadas → 20×401+5×429; `transportista-subir-archivo`, 35 POST → 30×400+5×429. `marketplace-api` y `data-api` deployadas en DEV por primera vez (antes solo existían en PROD). ✅ **EN DEV Y EN PROD** (deploy `v1.230.0`, 2026-09-22, PR #356, merge `f8d0ae3e`) — verificado con tráfico real contra PROD: 70 requests a `marketplace-api` → 64×403+6×429, reconciliando exacto contra la tabla. Ver [[wiki/architecture/guards-server-side]] ("Tanda G — backlog") y [[wiki/architecture/edge-functions]]. |
| 431 | `431_otp_entrega_y_vencimiento_link_cuenta.sql` | 🛡️ **Tanda G (2ª parte) — OTP de entrega y vencimiento del link de cuenta** (2026-09-20, commit `17171223`). **OTP**: el código salía de `random()` de Postgres, que no es criptográfico → ahora `extensions.gen_random_bytes` (pgcrypto vive en el schema `extensions`, por eso va calificado: las funciones tienen `search_path='public'`); verificado en DEV con 500 códigos distintos de 500. Columnas nuevas `envio_otp.intentos` e `envio_otp.invalidado_at`: **máximo 5 intentos** (antes ninguno, y `verificar_otp_envio` tiene GRANT a `anon` → 1.000.000 de combinaciones fuerza-bruteables), con `FOR UPDATE` para que el límite no se saltee con pedidos en paralelo, y un solo código activo por envío. **Link de estado de cuenta**: `clientes.cuenta_token` no vencía nunca y no se podía rotar, mientras `get_cuenta_cliente_by_token` (SECURITY DEFINER, GRANT a `anon`) devuelve nombre, teléfono, email y toda la CC → nuevas `clientes.cuenta_token_creado_at` y `tenants.cuenta_token_dias` (default **90**, 0 = no vence), chequeo dentro de la función (devuelve NULL igual que un token inexistente, para no confirmar que existió) y **trigger `clientes_cuenta_token_fechado`**, porque fecharlo solo desde el front dejaba los links NUEVOS sin fecha → el chequeo (que exige `IS NOT NULL`) no se cumplía nunca y el link volvía a ser eterno, en silencio (lo marcó el `migration-reviewer`). Los tokens ya existentes se fechan HOY, no en el pasado. ✅ **EN DEV Y EN PROD** (deploy `v1.229.0`, 2026-09-22). Verificada en DEV: intento malo=false, código correcto tras 5 fallidos=false, código nuevo=true; el trigger fecha el token nuevo y no re-fecha al tocar otra columna. En PROD el impacto de datos fue **cero** (0 clientes con link, 0 OTPs). ⚠️ Ojo: `envio_otp.intentos` (adivinar el código) y `envios.intentos` (reintentos de entrega, mig 190) son homónimas con semántica distinta. Ver [[wiki/architecture/guards-server-side]] ("Tanda G"). |
| 430 | `430_storage_aislamiento_por_negocio_y_search_path.sql` | 🛡️ **Tanda G — cierra la fuga real de Storage encontrada en la auditoría de seguridad del 2026-09-20** (commit `f55fbf0f`). `archivos-biblioteca`: las policies de SELECT y DELETE tenían condición SIEMPRE TRUE (nunca miraban `name`, donde vive la carpeta del tenant) — **un usuario del negocio A leía archivos del negocio B**, fuga de lectura real verificada en DEV. `productos`: INSERT/UPDATE pasaban de `auth.uid() IS NOT NULL` a secas a carpeta-del-propio-tenant + rol de gestión (se podía pisar fotos de otro negocio). `fn_enqueue_tn_fulfillment_sync` (SECURITY DEFINER) fijado con `search_path`. Verificado post-fix con 6/6 chequeos (ataques bloqueados, lo legítimo intacto). ✅ **EN DEV Y EN PROD** (deploy `v1.229.0`, 2026-09-22). Ver [[wiki/architecture/guards-server-side]] ("Tanda G"). |
...
| 426 | `426_soporte_consultas_desde_app.sql` | 💬 **Ayuda: "Reportar un problema" + Mis consultas** (pedido de GO). `support_tickets.usuario_id` (quién la abrió desde la app; FK a `users` `ON DELETE SET NULL`, NULL = ticket interno del equipo), `tipo` (problema/consulta/sugerencia/pago), `modulo`, `pendiente_equipo` (marca del panel), `ultimo_mensaje_at`; `support_messages.interno` (nota del equipo, el cliente no la ve) y `adjuntos` jsonb. Backfill de tickets existentes. Trigger `trg_support_message_actualiza_ticket`: mensaje del cliente → pendiente del equipo (reabre si estaba esperando/resuelta); respuesta de agente sin nota interna → deja de estar pendiente. `fn_notificar_respuesta_soporte` (de la 425) redefinida: ignora notas internas, avisa a `usuario_id` y lleva a `/ayuda/consultas?ticket=`. RPC SECURITY DEFINER con guard: `fn_soporte_crear_consulta`, `fn_soporte_responder` (no en cerradas), `fn_soporte_mis_consultas`, `fn_soporte_consulta` (sin notas internas, agente firma "Soporte Genesis360"); helper `fn_soporte_ve_todas_del_negocio()` (DUEÑO/SUPER_USUARIO ven todas las del negocio). Topes anti-spam (10 consultas/usuario/24h, 30 mensajes/hora) con `pg_advisory_xact_lock`. Bucket privado `soporte-adjuntos` (png/jpeg/webp/pdf, 5 MB) + policies `soporte_adjuntos_insert`/`soporte_adjuntos_select` por carpeta `<negocio>/<usuario>/`. `migration-reviewer`: APTA. Ver [[wiki/support/plataforma-soporte]]. |
| 425 | `425_notif_respuesta_ticket_soporte.sql` | 🔔 **La respuesta de soporte le llega al cliente** (pedido de GO). Trigger `trg_notificar_respuesta_soporte` (AFTER INSERT en `support_messages`, `WHEN autor_tipo = 'agente'`): notificación a la campanita del usuario que abrió el ticket (autor del primer mensaje `cliente`, si sigue en ese negocio) con "Soporte respondió: {asunto}" y el texto recortado a 500 caracteres. Tickets abiertos por el equipo = hilo interno, no avisa. BEGIN/EXCEPTION: un aviso que falla no frena la respuesta. De paso, REVOKE ALL de `anon`/`authenticated` en `support_tickets` y `support_messages` (solo los frenaba la RLS sin policies). Ver [[wiki/support/plataforma-soporte]]. |
| 424 | `424_aviso_precio_no_publicado_marketplace.sql` | 📣 **Aviso al dueño cuando ML o TN no toman un precio nuevo** (D2 del relevamiento de precio programado). Trigger `trg_notificar_sync_precio_fallido` (AFTER UPDATE OF status en `integration_job_queue`, `WHEN` el `sync_precio` pasa a `failed`): notifica a DUEÑO/SUPER_USUARIO con el producto, el canal y el último error. Si ya tienen un aviso sin leer del mismo canal de las últimas 6 horas, lo actualiza ("N productos…") en vez de sumar otro. Todo el cuerpo en BEGIN/EXCEPTION: un aviso que falla no frena al worker. Ver [[wiki/features/productos]]. |
| 423 | `423_precio_programado_repositor_anticipado.sql` | 🏷️ **Precio programado, Fases 2-3: la etiqueta de la góndola.** `tenants.repositor_anticipacion_min` (0-1440, default 60) y `tareas_repositor.precio_programado_id` + `vigente_desde`. `fn_generar_tareas_precio_programado` (la llama el cron de cada minuto antes de aplicar) crea la tarea anticipada con el precio nuevo, o la fusiona con una tarea de cartel sin hacer. `fn_generar_tarea_repositor_precio`: un cambio manual antes de la hora no pisa la etiqueta del programado. `fn_aplicar_precios_programados` liga la tarea al aplicar y su rama de error ahora tiene subtransacción propia. `fn_tareas_precio_programado_desarmar` al cancelar, reemplazar o fallar. Guard `fn_tarea_repositor_guard_completar`. 🛑 `authenticated` sin INSERT/DELETE en `tareas_repositor` y UPDATE solo por columna (antes podía reescribir tareas y saltear el guard). `vw_tareas_repositor` + `precio_programado_id`, `vigente_desde`, `precio_vigente`. CREATE OR REPLACE desde `pg_get_functiondef`. Ver [[wiki/features/productos]] y [[wiki/features/repositores]]. |
| 422 | `422_precios_programados.sql` | 🗓️ **Precio de venta con fecha/hora de vigencia — Fase 1** (relevamiento de Fede respondido por GO). Tabla `precios_programados` (un pendiente por producto, índice único parcial; RLS solo SELECT del negocio, sin escritura directa). `fn_programar_precio` (valida `auth_puede_editar_modulo('inventario')`, producto del negocio, fecha ≥ 1 min en el futuro y ≤ 1 año; reemplaza el pendiente anterior; registra en `actividad_log`) y `fn_cancelar_precio_programado`. `fn_aplicar_precios_programados` corre **cada minuto por pg_cron**: hace el mismo `UPDATE OF precio_venta` que un cambio manual (dispara la tarea del repositor y la cola de ML/TN), marca aplicado con el precio anterior y lo registra; si falla lo marca `fallido` y avisa al DUEÑO/SUPER_USUARIO. `fn_notificar_precios_programados_manana` (cron diario 12:00 UTC) avisa el día anterior. Ver [[wiki/features/productos]]. |
| 421 | `421_notif_cc_oc_vencidas_al_dueno.sql` | 🛑 **Los avisos diarios de CC y OC vencidas nunca le llegaban al dueño**: `fn_notificar_cc_vencidas` (mig 091) buscaba destinatarios con rol `OWNER`/`ADMIN`; `OWNER` no existe y `ADMIN` es staff. En PROD no le llegó nunca a nadie; en DEV, solo al usuario ADMIN (58 avisos en 30 días). Pasa a DUEÑO y SUPER_USUARIO. CREATE OR REPLACE desde `pg_get_functiondef`. |
...
| 406 | `406_recurso_activa_al_pagar_gasto.sql` | 🐛 **Issue #12/#13 de Fede.** Cierra el ciclo del módulo Recursos: al saldar el gasto de adquisición, el recurso pasa de `pendiente_adquisicion` a `activo`. Va en TRIGGER y no en `GastosPage` porque un gasto se salda por varios caminos (alta del pago, edición, pago de OC, sweep) y en una sola pantalla el recurso quedaría colgado en "pendientes" para siempre. Acotado: solo en la transición a 'pagado', solo sobre recursos pendientes (uno dado de baja **no** se revive), y sin camino inverso. |
| 405 | `405_modulo_gastos_server_side.sql` | ✅ **Tanda F, cierre definitivo.** Con la regla de negocio de GO: *un cajero registra un pago a proveedor SOLO si el dueño le dio el módulo Gastos en su rol custom*. No hizo falta estructura nueva — `auth_puede_editar_modulo` ya mira **primero** `roles_custom.permisos ->> 'gastos'`. Allowlist de roles fijos: **SUPERVISOR + CONTADOR** (verificado contra las rutas permitidas/restringidas de los specs 13/15/16/17/18). Gateadas: `proveedor_cc_movimientos`, `gastos_fijos`, `gasto_cuotas` y **`cheques`** — este último corrige un supuesto falso de la 404 ("el cajero crea cheques al cobrar": el POS **no** escribe cheques). ⚠ `cheques` va **comando por comando**: las policies permisivas se combinan con **OR**, así que un `FOR ALL` habilitaría el DELETE y la policy "solo gestión" no lo restaría. |
| 404 | `404_matriz_escritura_plata_precios_inventario.sql` | 🔴 **Tanda F1, cierre — la matriz de ESCRITURA.** Un CAJERO escribía por REST `cheques`, `cliente_creditos`, `caja_traspasos`, `producto_precios_mayorista`, `cupones`, `combos`, `sucursales`, `ubicaciones`, `estados_inventario`, `canales_venta`, `cuentas_origen`, `kit_recetas` y `proveedor_cuentas_bancarias` (el CBU al que se le paga a un proveedor). **`producto_precios_mayorista`/`combos`/`cupones` eran el precio de venta que cerró la mig 396, por la puerta de al lado.** El corte va por **OPERACIÓN**: config → gestión · precios/comercial → `auth_puede_editar_modulo` (que suma el módulo `comercial`) · `kit_recetas` → inventario + DEPÓSITO · `cliente_creditos` → INSERT operativo, UPDATE/DELETE gestión · `cheques` → registrar/cobrar/endosar operativo, **MONTO** por trigger de columna y DELETE gestión · `caja_traspasos` → INSERT operativo, UPDATE (corrección) DUEÑO/SUPERVISOR. ⚠ `cupones_codigos` NO se cierra por tabla: **el POS lo escribe al canjear**. |
| 403 | `403_secretos_restantes_y_rrhh_items.sql` | 🔴 **Tanda F2, cierre.** Auditoría repetida sobre TODO el esquema (111 de 152 tablas no miran el rol). (1) La mig 400 dejó afuera **Mercado Libre** (`access_token`+`refresh_token`), **MODO** (`api_key`) y **couriers** (`credenciales` JSONB) — los tres legibles por cualquier rol, y por `anon`. (2) La mig 401 dejó afuera el **detalle** de la liquidación: `rrhh_salario_items` (26 filas visibles para un CAJERO en DEV) y `rrhh_anticipos` — con la cabecera cerrada y el detalle abierto, el sueldo se reconstruye sumando conceptos. (3) La **escritura** de las 6 tablas de credenciales pasa a DUEÑO/ADMIN/SUPER_USUARIO: antes un CAJERO podía **desconectar las integraciones** o pisar un token. Columna generada `credenciales_configuradas` para que el panel de couriers siga diciendo "Configurado" sin recibir el secreto. 0 filas en PROD al momento de aplicar. |
| 402 | `402_nucleo_fiscal_server_side.sql` | 🔴🔴 **Tanda F, cierre — lo más grave de la tanda.** `emisores_fiscales`, `tenant_certificates` y `puntos_venta_afip` tenían UNA policy `FOR ALL` que solo miraba el tenant: **cualquier rol** podía cambiar el CUIT / la condición de IVA / el umbral de Factura B, prender `afip_produccion` (CAE fiscal REAL), borrar el certificado AFIP y tocar los puntos de venta. Y del bucket `certificados-afip` se **descargaba la clave privada** (su INSERT era `auth.uid() IS NOT NULL` a secas → se escribía en la carpeta de OTRO tenant). Ahora: SELECT para todo el tenant (el POS lo necesita) + escritura solo DUEÑO/ADMIN/SUPER_USUARIO. Además `afipsdk_token` pasa a **secreto de solo escritura** (sin SELECT ni para el DUEÑO; columna generada `afipsdk_token_configurado` para la UI) y la copia legacy `tenants.afipsdk_token` se vacía, sale del espejo de la mig 271 y queda forzada a NULL por trigger. Ver [[wiki/features/facturacion-afip]], [[wiki/features/multi-cuit]]. |
| 401 | `401_rrhh_visibilidad_por_rol.sql` | 🔴 **Tanda F2.** Sueldo, CBU y DNI de cada empleado los leía **cualquier rol**. RLS por fila con la regla aprobada por GO (DUEÑO/ADMIN/SUPER_USUARIO/RRHH todo · SUPERVISOR su equipo · cada empleado lo suyo) + `fn_empleados_basico()` (sin datos sensibles) y `fn_sueldos_agregado()` (totales) para las 5 pantallas que leían esas tablas. |
| 400 | `400_credenciales_integraciones_sin_token.sql` | 🔴 **Tanda F2.** Los `access_token` de Mercado Pago, Tienda Nube y WhatsApp **los leía cualquier rol del tenant** en claro. Privilegios a nivel COLUMNA: se revoca el SELECT de tabla y se re-otorga sin los secretos. `service_role` intacto (Edge Functions). Ver [[wiki/architecture/guards-server-side]]. |
