...
| 204 | `204_rrhh_fichado_qr.sql` | **RRHH — fichado por QR público (v1.51.0, SOLO DEV)** · `tenants.fichado_token` (+ índice único parcial) + RPCs `get_fichado_info(text)` / `fichar_qr(text,uuid)` **SECURITY DEFINER** (`SET search_path=public`) con **GRANT EXECUTE a anon, authenticated** (kiosco /fichar/:token). Auto-descuento de tardanza + portal del empleado = solo frontend (columnas/config ya existían). Aditiva. **⚠ En DEV; pendiente PROD** |
