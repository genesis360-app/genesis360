...
**🗂️ Migración 426 — ✅ EN DEV · ⏳ PROD pendiente** (2026-09-15). Revisada por `migration-reviewer`: APTA, con sus 2 sugerencias aplicadas antes de aplicarla (lock en los topes anti-spam, chequeo de usuario activo en las RPC de lectura). Verificada en DEV: las 4 RPC (`fn_soporte_crear_consulta`/`fn_soporte_responder`/`fn_soporte_mis_consultas`/`fn_soporte_consulta`) SECURITY DEFINER + `search_path`, `EXECUTE` solo para `authenticated` (sin `anon`); `support_tickets`/`support_messages` siguen sin privilegios de tabla (heredado de la 425); acentos correctos. SQL con los triggers reales, impersonando al cajero: nota interna → 0 avisos y no aparece en su hilo; respuesta del equipo → aviso con `action_url` a la consulta; consulta resuelta que el cliente reabre al escribir; cerrada rechaza responder. Bucket `soporte-adjuntos` + 2 policies de storage verificadas (propio, o DUEÑO/SUPER_USUARIO del negocio). Sin dependencia de orden con el frontend actual (aditiva), pero **antes de redesplegar `billing-manual-avisar-pago`** (inserta columnas nuevas). Ver [[wiki/support/plataforma-soporte]].

**🗂️ Migración 425 — ✅ EN DEV · ⏳ PROD pendiente** (2026-09-15). Verificada en DEV con el trigger real dentro de un bloque que se revierte: ticket del cliente → 2 avisos (el largo recortado a 498), ticket interno → 0, autor de otro negocio → 0; sin la migración daba 0 (mutante). SECURITY DEFINER + `search_path`, sin `EXECUTE` para `anon`/`authenticated`, acentos correctos; `support_tickets`/`support_messages` quedan solo para `postgres` y `service_role` (PostgREST con anon → 401). Sin dependencia con el frontend de la app; el cambio del panel (`genesis360-admin`, commit en `dev`) es solo un texto.

**🗂️ Migraciones 423-424 — ✅ EN DEV · ⏳ PROD pendiente** (2026-09-15). `migration-reviewer`: APTAS (se aplicó su recomendación antes de aplicarlas). 423 verificada: las 7 funciones con SECURITY DEFINER + `search_path`, `EXECUTE` para `authenticated` solo en programar y cancelar, `tareas_repositor` sin INSERT/UPDATE/DELETE de tabla para `authenticated` y UPDATE en 6 columnas, vista con `security_invoker` y las 3 columnas nuevas al final, trigger del guard, acentos (`DUEÑO` = `c391`) y el cron activo. 424 verificada: SECURITY DEFINER, sin `EXECUTE` para `anon`/`authenticated`, trigger con su `WHEN`. Smoke de PostgREST con control negativo (columna inventada → 400). **Orden en el deploy:** después de la 422 y antes del frontend. Sin la 423, Alertas no muestra las etiquetas vencidas y Config no guarda la anticipación, pero nada se rompe.

**🗂️ Migraciones 421-422 — ✅ EN DEV · ⏳ PROD pendiente** (2026-09-14). 421 verificada: las dos consultas de destinatarios usan DUEÑO/SUPER_USUARIO, SECURITY DEFINER + `search_path`. 422 verificada: tabla con RLS (solo SELECT del negocio, `anon` sin privilegios), `fn_programar_precio` y `fn_cancelar_precio_programado` ejecutables solo por `authenticated`, `fn_aplicar_precios_programados` y el aviso del día anterior solo por el cron, los dos jobs activos y acentos correctos. **Sin dependencia de orden con el frontend** (la 422 es aditiva): conviene aplicarlas antes del merge.
