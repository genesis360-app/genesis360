...
🔒⏱️ **Migración 432 — ✅ EN DEV, 🔴 falta en PROD** (`v1.230.0`, 2026-09-22): rate limiting persistente para
las Edge Functions públicas. Cierra el pendiente 3 del backlog de la auditoría de seguridad del 2026-09-20
— `marketplace-api`, `data-api` y `transportista-subir-archivo` llevaban la cuenta en un `Map` en memoria
del isolate de Deno, que se pierde en cada cold start y no es compartido entre los isolates en paralelo que
corre Supabase de la misma función. Tabla nueva `rate_limit_contadores` (sin `tenant_id` a propósito) +
`fn_rate_limit_consumir` (atómica, `SECURITY DEFINER`, `EXECUTE` solo `service_role`) + cron horario de
limpieza. De paso se cerraron 2 hallazgos nuevos: el límite se esquivaba falseando `x-forwarded-for`, y en
`data-api` probar API keys al azar no chocaba con ningún tope. Verificado en DEV con tráfico real (60/70
pasan en `marketplace-api`, 20×401+5×429 en `data-api`, 30×400+5×429 en `transportista-subir-archivo`).
`schema_full.sql`: **171 tablas** (era 170), policies siguen en 234 (la tabla es deny-all, no agrega
ninguna). Detalle en [[wiki/architecture/guards-server-side]] ("Tanda G — backlog") y `log.md`
(2026-09-22, `update`).
