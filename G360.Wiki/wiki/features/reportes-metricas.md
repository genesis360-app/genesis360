...
- **Áreas de módulo** (Ventas/Gastos/Productos/Inventario/Clientes/Proveedores/Facturación/Envíos/Marketing):
  - **Insights / Métricas / Gráficos** → el componente `DashXArea` con la prop `section` (`insights|metricas|graficos`) que gatea sus 3 bloques ya existentes (insights / KPIs / charts). Tipo en `src/components/dashAreaSection.ts`. Prop `embedded` (v1.93.0) oculta la barra de filtros/banners propios.
  - **Filtro global UNIFICADO (v1.94.0):** un solo control Período/Moneda (arriba a la derecha) gobierna las áreas **con período** — `AREAS_CON_PERIODO = Todo/Ventas/Gastos/Productos` — en las sub-pestañas de datos (Gráficos/Insights/Métricas). Esas 3 áreas se renderizan `embedded` y toman el período/moneda del filtro global vía props `gPeriodo/gMoneda/gCustomDesde/gCustomHasta` → helpers compartidos `getFechasDashboard`/`getFechasAnteriores`. El filtro **no se muestra** en las áreas de período fijo ni en Rentabilidad/Recomendaciones (no haría nada). **Las 6 áreas de período fijo** (Inventario/Clientes/Proveedores/Facturación/Envíos/Marketing) son snapshots por diseño (mes actual / últimos N días / 12 meses) → **no se embeben en standalone**, conservan sus controles propios (ej. toggle **Vista** de Inventario [Todo/Mercadería/Recursos] y Proveedores [Consolidado/Mercadería/Servicios]). En el agregado de "Todo › Gráficos" sí se embeben (vista compacta).
