...
**🎉 RRHH 2.0 (RH1-RH8) COMPLETO.** Diferidos (mejoras futuras, no en el plan original): el **fichado por QR público** (`/fichar/:token`) y el **auto-descuento de tardanza** inyectado en la nómina (RH6; la lib `descuentoTardanza` ya existe, falta el sweep) y la **UI completa del portal del empleado** (F2; el flag `tenants.rrhh_portal_empleado` ya está configurable). Confirmado por GO: % de aportes editables en Config, categorías Sueldos/Cargas sociales, prorrateo del básico por frecuencia, indemnización LCT 245 editable.
