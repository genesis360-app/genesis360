...
- **Sin CAE** (`despachada`): mismo efecto de siempre — `cambiarEstado.mutate({nuevoEstado:'cancelada'})` (reincorpora stock + revierte caja proporcional a los medios originales) — se ejecuta directo al aprobar, sin pedir nada nuevo (igual criterio que Clientes).
