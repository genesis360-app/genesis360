...
elegirCotizacionReintegro(usarCotizacionOriginal, cotizacionVentaOriginal, cotizacionHoy) // mig 375 (Fase 6 Caja USD, G1): cotización del reintegro en USD de una devolución — hoy por default, o la de la venta original si el tenant lo configura
...
- **J2**: **clave maestra** (RPC `verificar_clave_maestra`) para **cambiar cliente** (botón en el detalle) y **override de descuento** (autoriza descuentos sobre el tope de rol/canal, re-ejecuta la venta con `overrideDescuento`). Sin `tenants.clave_maestra` configurada no se exige. **"Anular venta despachada" migró a cola de Supervisión (A1, v1.194.0) — ver VF6, ya NO usa clave maestra.**
...
**La regla nueva (NC-antes-de-eliminar) se resuelve por rama, al APROBAR** (siempre re-fetchea la venta fresca, nunca confía en el estado de cuando se pidió):
- **Sin CAE** (`despachada`): mismo efecto de siempre — `cambiarEstado.mutate({nuevoEstado:'cancelada'})` (reincorpora stock + revierte caja: cada parte vuelve por donde entró — pesos netos del vuelto a la caja en pesos, dólares a la Caja USD, informativos por medio; ver [[wiki/features/caja]], fix del 2026-09-14) — se ejecuta directo al aprobar, sin pedir nada nuevo (igual criterio que Clientes).
- **Con CAE** (`facturada`): **antes el botón "Anular" ni se ofrecía** (bloqueaba del todo, indicaba usar "Devolver"). Ahora la solicitud SÍ se ofrece, y **aprobarla abre "Devolver" automáticamente, precargada a devolución TOTAL** (todos los ítems al máximo + todas las series preseleccionadas — `abrirModalDevolucion(venta, {anulacionTotal:true})`, nuevo 2º parámetro opcional, sin tocar los 2 call-sites existentes). El **supervisor elige el reembolso ahí mismo** (nunca se automatiza un reembolso fiscal sin intervención humana — REGLA #0); al confirmar, dispara la NC automática (**A10**, ver [[project_nc_afip_automatica]]) sin cambios. La autorización queda `aprobada` recién cuando `procesarDevolucion` cierra al **100%** del total (mismo chequeo que ya usaba para marcar `estado='devuelta'`) — si es parcial o se cancela el modal, sigue `pendiente`.

**"Cambiar cliente" no cambió**: sigue con clave maestra, sigue oculto con CAE (mismo motivo de siempre — el comprobante ya está en AFIP a nombre de un cliente fijo).
