...
import {
  ...
  BarChart2, Users, Settings, LogOut, Menu, X, ChevronRight, ShoppingCart
} from 'lucide-react'
    ...
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {navItems.map(({ to, icon: Icon, label, badge, ownerOnly }) => {
          if (ownerOnly && user?.rol !== 'OWNER' && user?.rol !== 'SUPER_USUARIO') return null
          return (
            <NavLink
              key={to}
              to={to}
      ...
      {/* Usuario y logout */}
      <div className="px-3 py-4 border-t border-accent/20 space-y-1">
    ...
    <div className="flex h-screen bg-brand-bg overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="lg:hidden flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-200 shadow-sm">
          ...
          <span className="font-bold text-primary">StockApp</span>
        </header>
