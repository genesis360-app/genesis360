        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-accent/30 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              ...
              <span className="text-gray-400 font-normal ml-1">({form.estados.length} seleccionado{form.estados.length !== 1 ? 's' : ''})</span>
            </label>
            {estados.length === 0 ? (
              <p className="text-sm text-gray-400">No hay estados creados aún</p>
            ) : (
          ...
          <label className="flex items-center gap-3 cursor-pointer p-3 bg-amber-50 border border-amber-200 rounded-xl">
            <div className="relative">
              ...
              <div className={`w-10 h-5 rounded-full transition-colors ${form.es_default ? 'bg-amber-500' : 'bg-gray-300'}`}>
                <div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${form.es_default ? 'translate-x-5' : ''}`} />
              </div>
            </div>
            <div>
              <p className="text-sm font-medium text-amber-800">Marcar como filtro por defecto</p>
              <p className="text-xs text-amber-600">Este grupo aparecerá preseleccionado en Rebaje y Ventas</p>
            </div>
          </label>

          <div className="flex gap-3 justify-end">
            <button onClick={resetForm}
              className="px-5 py-2.5 border-2 border-gray-200 text-gray-600 font-semibold rounded-xl hover:border-gray-300 text-sm">
              Cancelar
            </button>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {grupos.map(grupo => {
            ...
            return (
              <div key={grupo.id} className={`bg-white rounded-xl p-4 shadow-sm border transition-all
                ${grupo.es_default ? 'border-amber-300 bg-amber-50/30' : 'border-gray-100'}`}>
