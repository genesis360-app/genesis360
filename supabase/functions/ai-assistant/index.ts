...
// ── Plan IA, Fase 3 (memoria persistente por tenant) — espejo en src/lib/aiAssistant.ts ────
// Guarda HECHOS DESTILADOS del negocio (no charla cruda), con confirmación explícita del
// usuario (misma tarjeta que la propuesta de config) — nunca se escribe nada sin que el usuario
// haga click en "Confirmar". Mismo gate de rol que la propuesta de config (fn_ai_memoria_guardar,
// mig 377, exige DUEÑO/ADMIN igual).
function construirToolGuardarMemoria() {
  return {
    type: 'function',
    function: {
...
interface HechoMemoriaValido { ok: true; hecho: string }
interface HechoMemoriaInvalido { ok: false; error: string }

function validarHechoMemoria(args: { hecho?: unknown }): HechoMemoriaValido | HechoMemoriaInvalido {
...
export function construirSystemPrompt(ctx: ContextoUsuario | undefined, textoUsuario: string, memoria: string[] = []): string {
  const partes: string[] = []
...
serve(async (req) => {
  ...
  // Resolver el tenant EXPLÍCITAMENTE una sola vez, acá arriba (no confiar en RLS desnuda de
  // `tenants`): para un rol ADMIN (staff de plataforma) la policy `tenants_select` agrega "OR
  // is_admin()", que sin un .eq('tenant_id', ...) explícito devolvería filas de TODOS los
  // tenants — se reutiliza este mismo tenantId más abajo, si corresponde, para el valor actual
  // de una propuesta de config (antes se resolvía dos veces).
  const { data: userRow } = await supabase.from('users').select('tenant_id').eq('id', user.id).single()
  const tenantId: string | null = userRow?.tenant_id ?? null

  // Plan IA, Fase 3 (memoria persistente) — hechos guardados en sesiones anteriores de ESTE
  // tenant, se inyectan en el prompt para personalizar la respuesta a CUALQUIER usuario del
  // negocio (leer no requiere DUEÑO/ADMIN, solo escribir — ver fn_ai_memoria_guardar, mig 377).
  // Vía RPC `fn_ai_memoria_listar` (SECURITY DEFINER, mig 378) y no un SELECT directo a la
  // tabla: la policy SELECT de `ai_tenant_memoria` exige DUEÑO/ADMIN/SUPER_USUARIO (mismo
  // umbral que el audit log de config) — con la sesión real de un CAJERO/DEPÓSITO/etc. esa
  // policy devuelve [] en silencio, y la personalización nunca llegaría a esos roles (hallazgo
  // real de code-reviewer sobre la primera versión de este fetch).
  const { data: memoriaRows } = await supabase.rpc('fn_ai_memoria_listar')
  const memoria: string[] = (memoriaRows ?? []).map((r: any) => r.hecho)
  ...
  const systemPrompt = construirSystemPrompt(contexto, textoUsuario, memoria)

  // Plan IA, Fases 2/3 (wiring) — ambas herramientas (proponer config / guardar memoria) solo se
  // ofrecen a quien de verdad puede confirmarlas (mismo gate que el prompt de arriba, las RPCs
  // exigen DUEÑO/ADMIN igual).
  const puedeProponerConfig = contexto?.rol === 'DUEÑO' || contexto?.rol === 'ADMIN'
  const tools = puedeProponerConfig ? [construirToolPropuestaConfig(), construirToolGuardarMemoria()] : undefined
  ...
  if (toolCall?.function?.name === 'proponer_cambio_configuracion' && puedeProponerConfig) {
    ...
    // tenantId ya resuelto arriba, explícitamente (evita el mismo problema de ADMIN/is_admin()
    // que dejaría `data` null en silencio si se confiara en RLS desnuda acá).
    const { data: tenantRow } = tenantId
      ? await supabase.from('tenants').select(propuesta.campo.campo).eq('id', tenantId).maybeSingle()
      : { data: null }
  ...
  // Plan IA, Fase 3 (wiring) — el modelo pidió guardar un hecho en la memoria del negocio. Se
  // valida (defensa en profundidad, la RPC fn_ai_memoria_guardar/mig 377 es la autoridad real)
  // pero NUNCA se escribe nada acá: el front la muestra como tarjeta de confirmación, y solo si
  // el usuario confirma se llama a la RPC.
  if (toolCall?.function?.name === 'guardar_hecho_memoria' && puedeProponerConfig) {
    let args: any = {}
    try { args = JSON.parse(toolCall.function.arguments ?? '{}') } catch { /* args queda {} → falla la validación de abajo */ }

    const hechoValidado = validarHechoMemoria(args)
    if (!hechoValidado.ok) {
      return new Response(JSON.stringify({
        reply: `No pude armar esa propuesta de memoria: ${hechoValidado.error} ¿Podés repetir el pedido?`,
      }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }

    return new Response(JSON.stringify({
      propuestaMemoria: { hecho: hechoValidado.hecho },
    }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
  }
