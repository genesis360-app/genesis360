  ...
  try {
    ...
    if (nuevoEstado === 'reservada' || nuevoEstado === 'pendiente') {
      for (const item of order.order_items ?? []) {
        ...
        // el stock de componentes en ubicaciones habilitadas para MELI, la RPC no hace nada y
        // la orden queda con el mismo faltante que tendría cualquier producto sin armar — nunca
        // arma en silencio (relevamiento D2, A2).
        if (remaining > 0) {
          const { data: prodKit } = await supabase.from('productos').select('es_kit').eq('id', mapped.producto_id).maybeSingle()
          if (prodKit?.es_kit) {
            const { data: tareaArmadoId, error: armadoErr } = await supabase.rpc('fn_iniciar_armado_kit_auto', {
              p_tenant_id: cred.tenant_id,
              p_kit_producto_id: mapped.producto_id,
              p_cantidad: remaining,
              p_canal: 'MercadoLibre',
              p_sucursal_id: cred.sucursal_id,
              p_origen_ref: String(orderId),
            })
            if (armadoErr) {
              console.error('Error generando armado automático de kit (no bloqueante):', armadoErr.message)
            } else if (tareaArmadoId) {
              console.log(`Armado automático generado para kit ${mapped.producto_id} (orden ML #${orderId}): tarea ${tareaArmadoId}`)
            }
          }
        }
