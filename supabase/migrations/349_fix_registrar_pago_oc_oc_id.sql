-- 349 — Fix REGLA #0: registrar_pago_oc (mig 237) filtraba orden_compra_items por una columna
-- `oc_id` que nunca existió ahí (la columna real es `orden_compra_id`) — bug dormido desde que se
-- creó la función, disparado SOLO cuando la OC no tiene `monto_total` seteado directo (fuerza el
-- fallback "Σ items" de la línea 78-81 original). Confirmado con datos reales en DEV: OC #30 del
-- tenant Almacén Jorgito tiene `monto_total IS NULL` y reproduce "column oc_id does not exist" al
-- intentar pagarla. Único cambio real: `WHERE oc_id = p_oc_id` → `WHERE orden_compra_id = p_oc_id`,
-- resto de la función textualmente idéntico a la mig 237.
...
COMMENT ON FUNCTION public.registrar_pago_oc(uuid,jsonb,numeric,text,uuid,jsonb,int,text) IS
  'Pago atómico y clave-gated de una OC (mig 237). Redefinida en mig 349: fix del bug real "column oc_id does not exist" — orden_compra_items usa orden_compra_id, no oc_id. Resto de la lógica idéntica.';
