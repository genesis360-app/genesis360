...
2. **FAC-27 CON/SIN** (L9): Factura B con `venta.total` ≥ `umbral_factura_b` — sin DNI/CUIT bloquea (UI y EF 400); con DNI emite DocTipo 96. Llamar también la EF directa para el guard server. — 🔴 **GAP doable** (requiere tenant RI con facturación; en Jorgito=Mono no aplica B → usar el guard UI `VentasPage:1490-1499` con un tenant RI o validar la EF directa).
