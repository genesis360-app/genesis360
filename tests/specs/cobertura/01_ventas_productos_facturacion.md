...
**✅ HECHO (math validada, sin riesgo): `prorratearDescuentoGlobal(items, totalObjetivo)`** en `facturacionLogic.ts` (espejo de la EF) + **6 unit tests de Factura B (FAC-DESC-01..06)** que prueban el bug (ImpTotal $2.000 sin prorrateo) y el fix (ImpTotal $1.800, IVA $312.40 sobre el neto descontado, NC consistente `precio×qty=subtotal`, redondeo exacto anti-10048, multi-alícuota, no-op sin descuento). **⏳ PENDIENTE (requiere validación que ahora está bloqueada):** (1) **cablear** la función en `registrarVenta` (que `venta_items` se guarden prorrateados) — toca la semántica de `precio_unitario`/display de PDF; (2) **smoke en homologación** = emitir una **Factura B con descuento general** + su **NC** y verificar `ImpTotal/ImpIVA` = monto descontado con CAE real (bloqueado por MCP caído + Jorgito es Monotributo → hace falta emisor RI de homologación, dominio AFIP de GO). **Mitigación operativa hasta el deploy:** no combinar "Descuento general" con facturación (usar descuentos por-ítem).
