...
| L28 | Crédito a favor: requiere cliente, no supera saldo disponible | `VentasPage.tsx:2438-2444` | 🔴 | ✅spec 53 (mutante) |
| L28b | Crédito a favor: auto-sugerencia en POS (v1.98.0) = `min(saldo,total)`, no pisa al usuario, no supera saldo ni total, presupuesto no consume | `VentasPage.tsx` efecto auto-sugerir + `saldoFavor.ts:montoSugeridoCredito` | 🔴 | ✅unit (saldoFavor ×6) · 🟡e2e (cubierto por guard spec 53) |
...
| `cliente_consumidor_final` | `true` | `VentasPage.tsx:146,2410,2414` | permite vender como CF (sin cliente) | `false`→cliente obligatorio siempre | ✅code-verified (`!permiteCF`→`clienteRequerido` en la misma cláusula que spec 60; gating UX no-REGLA#0) |
...
| `reserva_vencimiento_dias` | `null` | `liberar_reservas_vencidas` RPC; `VentasPage.tsx:660`; `ConfigPage.tsx:577,942` | sweep vence reservas viejas (libera stock + cancela) | null→no vencen | ✅DB (sim+ROLLBACK: reservada vencida → reservado 3→0 + cancelada). Ver L48 |
...
| `precio_redondeo` | `'none'` | `precioRedondeo.ts:redondearPrecio`; `VentasPage.tsx:2150-2151` (`precioTierEfectivo`); `ConfigPage.tsx:694,1014,2430` | redondea el precio unitario efectivo (subtotal/IVA/factura derivan de ahí) | `'none'`→sin cambios | ✅unit(precioRedondeo) · code-verified (H4 cerrado v1.82.0; nota previa "flag muerto" era STALE) |
...
| `reglas_canal` | `{}` | `useCanalesVenta.ts:53-68`; `VentasPage.tsx:2139-2141 (lista_precio), 2415 (requiere_cliente), 2378 (tope desc)` | por canal: tope descuento, lista mayorista/minorista, requiere_cliente, devolución_dias | sin reglas→sin restricción de canal | ✅e2e 61 (tope desc) · ✅code-verified: `requiere_cliente`→`clienteRequerido` (=spec 60) · `lista_precio` mayorista/minorista fuerza el tier en `precioTierBase`→`venta_items.precio_unitario` (toca plata, correcto) |
...
- **🛑 G0.6 — DESCUENTO GENERAL / MULTI-COMBO no se prorratea en los ítems → la FACTURA sobre-factura (REGLA #0 FISCAL, code-audit 2026-06-24, AVISADO A GO).** En `VentasPage.registrarVenta`, `venta_items.subtotal = getItemSubtotal()` (2295-2300) refleja solo el descuento **por-ítem**; el **descuento general** (`descTotalMonto`) y el **multi-combo** (`descCombosMulti`) reducen `venta.total` (`= subtotal − descTotalMonto − descCombosMulti`, línea 2312) pero **NO** se prorratean en los `venta_items`. La EF `emitir-factura` calcula `ImpTotal = Σ(ítems neto+IVA) = subtotal` (PRE-descuento) y solo hace `console.warn` (`index.ts:236-238`) — **no prorratea ni bloquea**. ⇒ **Toda venta con "Descuento general" o multi-combo que se facture emite el CAE por el monto SIN el descuento** → la factura (y su IVA discriminado en A/B) **sobre-factura** lo que el cliente realmente pagó. Riesgo: AFIP/contable (el cliente queda facturado por más de lo que pagó; el IVA débito queda inflado). **Reachable:** "Descuento general" se renderiza para `modoVenta!=='pendiente'` (despachada/reservada) y la facturación corre sobre la despachada. **Opciones de fix (decisión fiscal de GO):** (a) **prorratear el descuento en `venta_items.subtotal` al registrar la venta** (frontend-only, sin EF/migración; los ítems quedan reflejando lo pagado — afecta PDF/ticket/reportes, que pasan a mostrar el neto real); (b) **prorratear en la EF** antes de calcular neto/IVA (`factor = venta.total / Σítems`), dejando los ítems a precio de lista (cambio fiscal en la EF → exige smoke en homologación: emitir A/B con descuento general y verificar `ImpTotal/ImpIVA` = monto descontado). **Alcance REAL (más amplio de lo que parecía):** la **factura** suma `venta_items.subtotal` (incluye descuento por-ítem, NO el global) y la **NC** usa `precio_unitario × cantidad` (precio de LISTA — ignora descuento por-ítem **y** global). ⇒ es una inconsistencia de **representación de descuentos en toda la cadena factura↔NC**, no solo el global en la factura. La decisión de GO (2026-06-24): **opción (a)** — que el precio efectivo (post todos los descuentos) fluya por `venta_items` de modo que `subtotal = precio_unitario × cantidad` y la cadena entera (caja↔factura↔NC↔Libro IVA) quede consistente sobre UN número.
**✅ HECHO Y CABLEADO (decisión GO 2026-06-24: el precio efectivo va en `venta_items`):**
- **`prorratearDescuentoGlobal(items, totalObjetivo)`** en `facturacionLogic.ts` + **6 unit tests de Factura B (FAC-DESC-01..06)**: bug (ImpTotal $2.000 sin prorrateo) vs fix (ImpTotal $1.800, IVA $312.40 sobre neto descontado, NC consistente `precio×qty=subtotal`, redondeo exacto anti-10048, multi-alícuota, no-op sin descuento). 42/42 verdes.
- **Cableado en `VentasPage.registrarVenta`** (Fase 1 itemPayloads): cuando hay descuento general/multi-combo (`hayDescGlobal`), los `venta_items` se guardan con el **precio efectivo** (`precio_unitario`/`subtotal` prorrateados a `venta.total`, `descuento=0` para no aplicarlo dos veces). ⇒ factura (suma `subtotal`) **y** NC (usa `precio_unitario × cantidad`) salen por lo que paga el cliente; caja/CC/Libro IVA consistentes. **Es NO-OP para ventas sin descuento global** (no toca el caso normal) → bajo riesgo. typecheck + build verdes.
- **Validación AFIP (homologación) — Kiosco Buildi SÍ emite Factura B con CAE real** (CUIT 23-32031506-9; lo prueban las B previas #46-53). El smoke final end-to-end (vender con "Descuento general" → Factura B → CAE/PDF = descontado) se hace por la **app** (no por script: la persistencia de la EF falla bajo usuario sintético — ver hallazgos).
**✅ 2 hallazgos RESUELTOS en v1.89.0:**
- **NC/devolución con descuento POR-ÍTEM (combos)** → **RESUELTO:** la devolución arma `devItems` con el precio EFECTIVO pagado (`subtotal/cantidad`, `VentasPage:3138`) en vez del de lista → el reembolso a caja Y la NC acreditan lo realmente pagado (no de más). Cubre combos y descuento general, consistente con G0.6. No-op sin descuento.
- **EF no chequeaba el error del UPDATE post-CAE** → **RESUELTO:** `persistirCAE()` (`emitir-factura`) reintenta 3× y, si falla tras autorizar AFIP, **lanza un error con el CAE** (no `ok` en silencio) + `console.error`. EF en DEV+PROD. (La EF ya usaba `service_role` → el caso de fallo es transitorio, no RLS.)
- **✅ Medios de pago (L23) validados — spec 83:** 7 medios directos crean venta + caja correcta (efectivo→`ingreso`, no-efectivo→`ingreso_informativo`). Gotcha: no-efectivo exige monto EXACTO (sin vuelto).
...
7. ✅ **Crédito a favor** (L28) — **CERRADO: spec 53** (`53_credito_a_favor_excede_mutante`): cliente con $1 de crédito, aplicar $100 de "Crédito a favor" → bloquea ("No podés aplicar más que eso"), venta NO creada, crédito intacto ($1, sin consumir — verificado DB). *Falta (positivo): aplicar crédito válido descuenta `cliente_creditos` (Tanda B).*
   - **v1.98.0 — auto-sugerencia (L28b):** el POS auto-aplica "Crédito a favor" por `min(saldo, total)` al seleccionar un cliente con saldo. **REGLA #0 verificada contra el código:** `montoSugeridoCredito()` clampea a `min(saldo,total)` → nunca supera el saldo (respeta el guard de L2454) ni el total (`validarMediosPago` no genera vuelto falso); el consumo del ledger está gateado por `estado !== 'pendiente'` (L2869) → presupuesto no consume. No pisa pagos manuales (efecto solo actúa sobre medios vírgenes o sobre la línea que él mismo auto-aplicó, trackeada con `creditoAutoRef`). Cobertura: 6 unit (`saldoFavor.test.ts`) + el guard negativo ya cubierto por spec 53.
...
13. ✅ **`cliente_obligatorio`** (L20) — **CERRADO: spec 60** (`60_cliente_obligatorio_siempre_mutante`: `='siempre'` exige cliente hasta en una venta directa CF, que con el default `'reservas'` NO lo exige → aísla el flag manteniendo modo CF). ✅ **`cliente_consumidor_final=false`** code-verified (2026-06-24): `!permiteCF` está en la MISMA cláusula `clienteRequerido` (`VentasPage:2414`) → fuerza cliente; mismo efecto que spec 60, gating UX no-REGLA#0.
14. ✅ **`reglas_canal`** (L39) — **CERRADO (descuento): spec 61** (`61_reglas_canal_descuento_mutante`: `descuento_max_pct=5` por canal topea el descuento **incluso al DUEÑO** → gate de clave). ✅ **`requiere_cliente`** code-verified (`VentasPage:2415`, misma cláusula que spec 60). ✅ **`lista_precio`** code-verified (`VentasPage:2139-2141` `precioTierBase`: `'minorista'`→precio base; `'mayorista'`→mejor tier; fluye a `venta_items.precio_unitario`→subtotal/IVA/factura — **toca plata, correcto**). **Residual de Ventas CERRADO.**
