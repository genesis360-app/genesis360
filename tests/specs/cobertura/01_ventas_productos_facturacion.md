...
**🆕 2 hallazgos SEPARADOS (pre-existentes, a tratar aparte):**
- **NC con descuento POR-ÍTEM (combos):** sin descuento global, los `venta_items` siguen con `precio_unitario=lista`+`descuento=%`, y la EF arma la NC con `precio_unitario × cantidad` → **la NC de un ítem con descuento de combo acredita de más**. Mismo principio de fix (precio efectivo), pero aplicarlo a TODAS las ventas con combo cambia el display → decisión aparte.
- **EF no chequea el error del UPDATE post-CAE** (`emitir-factura/index.ts:354`): si el UPDATE de `ventas` falla después de que AFIP autorizó, queda **factura autorizada en AFIP sin registrar** → re-emisión posible (doble factura). Detectado al ver que en Buildi AFIP va adelante de la DB (máx 15 vs AFIP ≥17). En PROD sería grave. Endurecer: chequear el error + alertar/reintentar, nunca `ok` en silencio.
