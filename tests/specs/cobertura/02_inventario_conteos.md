...
| L20 | **Conteo — autorización de ajuste POR ROL** (DUEÑO directo / 'siempre' aprueba / 'umbral' delega al gate) | `ajusteAutorizacion.ts` (15-39); combinado `InventarioPage.tsx:1794` | 🛑 stock | ✅unit (ajusteAutorizacion) · ✅e2e 36 (solo rama DUEÑO directo) · 🔴 e2e rol≠DUEÑO (2 actores) |
...
5. **⭐ Reconciliación por delta con venta intercalada** (L21, `conteoAjuste.ts:reconciliarDelta` +
   `InventarioPage:1810`). Snapshot N → vender M durante el conteo → contar K → el ajuste debe ser
   `(vivo) + (K − N)` y **no** pisar la venta (verificar que no queda negativo ni se "recupera" el
   stock vendido). e2e 36 solo cubre el caso sin movimientos intermedios.
