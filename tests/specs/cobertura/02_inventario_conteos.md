...
| L13 | Kits — armar KIT consume componentes (recetas) | `iniciarArmado` (1294), `confirmarArmado` (1354), `cancelarArmado` (1414) | 🛑 stock | ✅code-verified 2026-06-24: **reservar** (valida disponible→throw; `cantidad_reservada`↑ FIFO; `kitting_log` en_armado con snapshot `componentes_reservados`) → **confirmar** (componentes `cantidad`↓ + reserva↓ + `movimientos_stock` rebaje; ingresa KIT + `movimientos_stock` kitting) → log completado; **cancelar** libera reserva sin consumir. Inverso del desarmar (e2e 75). ⚠️ obs: los writes del confirmar NO son transaccionales (patrón app-wide) → falla a mitad = stock parcial |
