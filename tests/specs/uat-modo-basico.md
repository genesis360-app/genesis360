...
| 12 | **Serializados: bloqueado a propósito** | Con `tiene_series` hay que decir QUÉ serie va a cada variante; repartir "6 y 4" a ciegas rompería la trazabilidad → la RPC lo rechaza con mensaje claro | revisión de código + guard en la RPC |
| 12-bis | 🛑 **Y NO se puede llegar a ese callejón** (mig 312, v1.144.1) | Hueco real que dejó la Fase 4: si se permitía convertir un serializado CON stock en agrupador, ese stock quedaba **atrapado** (no vendible porque la madre con hijos no se vende, no reasignable porque la RPC rechaza series). Ahora la conversión se bloquea en la UI **y** en el server (`trg_variante_compose_nombre`). Agregar otra variante a un agrupador que YA existe no se toca | verificado en DEV: serializado con stock → rechazado con el mensaje correcto; el mismo producto sin stock → variante creada con el nombre bien compuesto ("TEST312 Serial — Azul") |
