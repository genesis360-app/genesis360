...
| CAJ-37 | **Conversión USD↔$ — único punto de conversión del sistema, solo DUEÑO** (G5 Fase 5, F2, 2026-08-19) | (a) Usuario CAJERO/SUPERVISOR intenta acceder a "Convertir USD↔$"; (b) DUEÑO convierte USD→$; (c) DUEÑO convierte $→USD | (a) el botón no aparece para ningún rol que no sea DUEÑO (`convertir_usd_boveda` en `cajaPermisos.ts`, sin excepción de rol custom); (b) usa la cotización de COMPRA (`tenant.cotizacion_usd_compra`), descuenta de la Caja Fuerte USD y acredita en la Caja Fuerte ARS; (c) usa la cotización de VENTA (`tenant.cotizacion_usd`, la misma que el resto del POS), sentido inverso. Ambos casos: sin redondeo (decimales exactos), quedan auditados en `boveda_conversiones_usd` con la cotización efectivamente usada y los 2 `caja_movimientos` (egreso+ingreso) vinculados | H | 🔴 | ⬜ | |
