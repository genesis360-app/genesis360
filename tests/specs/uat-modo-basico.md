...
| VEN-24 | **Anular venta despachada (sin CAE) — vía cola de Supervisión, no clave maestra** (A1, 2026-09-01) | Detalle de venta despachada sin CAE → "Solicitar anulación" (motivo opcional) → Ventas → Autorizaciones → Aprobar | La venta queda `despachada` hasta que se aprueba (nunca se ejecuta al solicitar); al aprobar: **restaura stock** (reingreso mode-aware) exacto + revierte caja (`egreso_devolucion_sena` por el monto exacto cobrado) + cancela envíos pendientes; `autorizaciones` queda `aprobada` | E | 🔴 | ✅ | *E2E real 2026-09-01 contra DEV (spec 137, parte A): producto/venta propios, verificado por REST en cada paso (stock antes/después exacto, monto del egreso exacto, estado final `cancelada`)* |
| VEN-49 | **Anular venta FACTURADA (con CAE) — antes bloqueada del todo, ahora pasa por Supervisión + "Devolver"** (A1, 2026-09-01) | Detalle de venta facturada con CAE → "Solicitar anulación" → Ventas → Autorizaciones → Aprobar | Antes el botón "Anular" ni se ofrecía con CAE. Ahora sí se ofrece la solicitud; al **aprobar** se abre automáticamente "Devolver" **precargada a devolución total** (todos los ítems/series al máximo) — el supervisor elige el reembolso ahí mismo (nunca se automatiza), y al confirmar dispara la NC automática (A10). La autorización queda `aprobada` recién cuando la devolución cierra al 100%, nunca antes | E | 🔴 | ✅ | *E2E real 2026-09-01 contra DEV (spec 137, parte B) con CAE sintético (sin llamar a AFIP real): confirmado que el botón se ofrece, "Cambiar cliente" sigue oculto, la precarga es total (cantidad = máximo, total = precio de la venta), y cancelar sin confirmar deja la autorización `pendiente` y la venta sin cambios* |
...
| VEN-37 | **Pago combinado ARS+USD: cada caja recibe lo suyo** (G5 Fase 4, mig 372, 2026-08-18) | Con Caja ARS y Caja USD abiertas: vender un producto que acepta USD, cobrar parte en "Efectivo" y parte en "Efectivo USD" (monto exacto, sin vuelto) | La sesión ARS registra `ingreso` por la parte en pesos; la sesión USD registra `ingreso` por el monto en dólares REAL tipeado (no el equivalente convertido) — nunca un solo asiento mezclando ambas monedas | H | 🔴 | ⬜ | |
| VEN-38 | **🔴 CLAVE — Vuelto con sobrepago en USD sale en pesos, nunca en dólares** (G5 Fase 4, D1) | Total $100, cliente paga solo con "Efectivo USD" de más (equivalente a $200) | Vuelto de $100 se entrega en PESOS (nunca dólares); la Caja USD igual se acredita con el monto USD COMPLETO que se recibió; la Caja ARS registra un EGRESO por el vuelto (puede quedar en negativo neto si no había efectivo en pesos) | H | 🔴 | ⬜ | |
| VEN-39 | **A2 — producto sin flag no se puede cobrar en USD** | Cobrar en "Efectivo USD" un carrito con un producto `moneda_venta='local'` y `acepta_cualquier_moneda=false` | Bloquea la venta con mensaje claro; pide cobrar esa parte en pesos u otro medio. Con el flag activado (o el producto priceado en USD), sí se permite | H | 🔴 | ⬜ | |
| VEN-40 | **Cambiar el medio de pago de una línea resetea el monto** (G5 Fase 4, hallazgo de code-review) | Cargar "Efectivo" $5000 → cambiar el `<select>` de esa línea a "Efectivo USD" | El monto en pesos NO queda arrastrado; el input de dólares aparece vacío, obligando a tipear el monto real en USD. Sin este fix la venta podía "cobrarse" sin acreditar nada en ninguna caja | H | 🔴 | ⬜ | |
| VEN-41 | Cobrar en USD sin cotización cargada | Seleccionar "Efectivo USD" y tipear un monto, sin haber cargado la cotización del dólar | Bloquea con mensaje pidiendo cargar la cotización antes de cobrar en USD | E | 🟡 | ⬜ | |
| VEN-42 | Completar seña/saldo de una reserva en USD — no soportado todavía | Desde el modal de completar saldo de una reserva, elegir "Efectivo USD" y cargar un monto | Bloquea con mensaje claro pidiendo usar el POS principal (Nueva venta) — este flujo secundario todavía no tiene el input de dólares | B | 🟡 | ⬜ | |
| VEN-43 | Crear método de pago "Efectivo USD" | Config → Ventas → Métodos de pago → nuevo método, moneda USD + checkbox "Es efectivo real" | El método queda disponible en el selector del POS y participa como efectivo real (vuelto, saldo de caja, arqueo) en dólares | H | 🟡 | ⬜ | |
| VEN-44 | **Devolución con reintegro en "Efectivo USD" — cotización de HOY (default)** (G5 Fase 6, G1, 2026-08-19) | Devolver un ítem eligiendo "Efectivo USD" como medio, sin activar el toggle de Config | Aparece un input en dólares (con "≈ $X" de referencia); el egreso se asienta en la Caja USD elegida, en dólares reales — la conversión usa `tenant.cotizacion_usd` (la de hoy), sin importar qué cotización tuvo la venta original | H | 🔴 | ⬜ | |
| VEN-45 | **Devolución con reintegro en USD — cotización de la VENTA ORIGINAL (configurable)** (G5 Fase 6, G1, 2026-08-19) | Activar en Config → Caja en Dólares el toggle "usar cotización de la venta original", luego repetir VEN-44 sobre una venta con `cotizacion_usd` registrada | El monto en dólares se calcula con la cotización de la venta original, no la de hoy — igual que la NC. Si la venta no tiene cotización registrada, cae a la de hoy igual (sin bloquear) | H | 🟡 | ⬜ | |
| VEN-46 | **Guard: falta cotización cargada para devolver en USD** (G5 Fase 6) | Sin cotización cargada (`tenant.cotizacion_usd` en 0/null), tipear un monto en Efectivo USD y confirmar | Bloquea con "No hay cotización de dólar cargada..." — mensaje específico, no un descuadre de totales genérico | M | 🟡 | ⬜ | |
| VEN-47 | **Guard CAJ-18 aplicado a la Caja USD en devoluciones** | Intentar devolver más dólares de los que hay en la Caja USD elegida | Bloquea con el saldo disponible en USD, no permite dejar la Caja USD en negativo — mismo criterio que el guard ya existente para devoluciones en pesos | M | 🟡 | ⬜ | |
| VEN-48 | **NC AFIP de una devolución con componente USD sigue en pesos, a la cotización de la venta original** (G5 Fase 6, G2 — ya era así, no cambió) | Emitir la NC AFIP de una devolución de una venta con componente USD | La NC sale en pesos (`MonId:'PES'`), con el monto exacto que ya estaba declarado en la factura original — nunca se recalcula con la cotización de hoy ni con la que se usó para el reintegro en caja (G1, independiente) | H | 🔴 | ⬜ | |
...
| CAJ-31 | **Caja USD muestra "US$", nunca "$"** (G5 Fase 3, mig 371, 2026-08-18) | Abrir/operar/cerrar/arquear una caja con `cajas.moneda='USD'` | Todo monto de ESA caja/sesión (saldo, apertura, ingresos, egresos, arqueos, modales, ticket de cierre reimprimido desde el historial) se formatea en USD, nunca en pesos — antes `formatMoneda` siempre usaba `tenant.moneda` sin importar la moneda real de la caja abierta | H | 🔴 | ⬜ | |
| CAJ-32 | **Arqueo/cierre de Caja USD por denominación de billete** (G5 Fase 3, E2/J2, 2026-08-18) | Arqueo parcial o cierre de una Caja USD | En vez de un input de monto total, aparece un conteo por denominación (US$1/5/10/20/50/100); el total se calcula solo, solo billetes enteros (cantidades con decimales se truncan) | H | 🟡 | ⬜ | |
| CAJ-33 | **Umbral de diferencia propio en USD** (G5 Fase 3, E3, 2026-08-18) | Cerrar una Caja USD con diferencia | La alerta de diferencia en cierre compara contra `diferencia_caja_umbral_usd` (no el umbral en pesos) — un mismo número no dispara la misma alerta en ambas monedas | H | 🟡 | ⬜ | |
| CAJ-34 | **Bloquea traspaso entre cajas de distinta moneda** (G5 Fase 3, F1, mig 371) | Con una Caja ARS y una Caja USD abiertas, intentar traspasar de una a la otra | El selector de destino en el modal de traspaso NO ofrece cajas de otra moneda; si igual se fuerza el insert (ej. vía API), el trigger `fn_validar_traspaso_misma_moneda` lo rechaza con error explícito — verificado manualmente en DEV (ARS→USD rechazado, ARS→ARS permitido) | H | 🔴 | ✅ | *Verificado en DEV 2026-08-18 con INSERT directo dentro de una transacción con ROLLBACK (sin dejar datos): ARS→USD lanzó `P0001 No se puede traspasar entre cajas de distinta moneda`* |
| CAJ-35 | **Solo roles habilitados pueden operar Caja USD** (G5 Fase 3, I1, mig 371) | Usuario CAJERO (sin rol en `caja_usd_roles_permitidos`) entra a Caja | La Caja USD ni aparece en el selector de cajas operativas; DUEÑO siempre la ve y puede operarla. Si se fuerza el insert de todos modos, el trigger `fn_validar_rol_opera_caja_usd` lo rechaza | H | 🔴 | ✅ | *Verificado en DEV 2026-08-18 con INSERT directo (transacción + ROLLBACK): CAJERO rechazado con `P0001 Tu rol no tiene permiso para operar una Caja USD`, DUEÑO permitido* |
| CAJ-36 | **Bóveda con pestañas ARS/USD — cada moneda nunca se mezcla con la otra** (G5 Fase 5, mig 373, 2026-08-19) | (a) Pestaña "USD" en el tab Caja Fuerte; (b) selectores de "Ingresar"/"Enviar" a Caja Fuerte con la pestaña USD activa; (c) modo básico con la Caja USD como caja activa; (d) CAJERO solicita transferencia de su Caja USD; (e) aprobar esa solicitud | (a) cada tenant tiene 2 filas `es_caja_fuerte=true` (ARS/USD, sembradas automáticamente); cambiar de pestaña cambia saldo/movimientos/acciones sin mezclar monedas; (b) el `<select>` filtra a cajas de la MISMA moneda que la pestaña activa; (c) el botón "Depositar en Caja Fuerte" en modo básico fuerza la pestaña a la moneda de la sesión activa antes de abrir el modal; (d) el botón "Caja Fuerte" (solicitud) SÍ aparece para un CAJERO en Caja USD; (e) `aprobarSolicitudCajaFuerte` resuelve la Caja Fuerte de la MISMA moneda que la sesión que pidió la transferencia. 2 triggers server-side nuevos (`fn_validar_moneda_coincide_cuenta_origen`, defensa en profundidad) rechazan cualquier movimiento cuya moneda no coincida con su cuenta de origen | H | 🔴 | ⬜ | |
| CAJ-37 | **Conversión USD↔$ — único punto de conversión del sistema, solo DUEÑO** (G5 Fase 5, F2, 2026-08-19) | (a) Usuario CAJERO/SUPERVISOR intenta acceder a "Convertir USD↔$"; (b) DUEÑO convierte USD→$; (c) DUEÑO convierte $→USD | (a) el botón no aparece para ningún rol que no sea DUEÑO (`convertir_usd_boveda` en `cajaPermisos.ts`, sin excepción de rol custom); (b) descuenta de la Caja Fuerte USD y acredita en la Caja Fuerte ARS; (c) sentido inverso. ⚠️ Desde D-1 fase 2 (2026-09-26, §70) los dos sentidos usan la MISMA tasa (vendedor divisa BNA del día hábil anterior) — antes (b) iba a COMPRA y (c) a VENTA. Ambos casos: sin redondeo (decimales exactos), quedan auditados en `boveda_conversiones_usd` con la cotización efectivamente usada y los 2 `caja_movimientos` (egreso+ingreso) vinculados | H | 🔴 | ⬜ | |
| CAJ-38 | **Retiro de Caja USD sin destino requiere clave maestra** (G5 Fase 5, F3, 2026-08-19) | DUEÑO extrae dinero de la cuenta "Efectivo USD" desde "Extraer dinero" (bóveda), con clave maestra configurada en el tenant y monto ≥ `caja_usd_clave_maestra_umbral` | Aparece un campo "Clave maestra" adicional (además del motivo, ya obligatorio); el botón de confirmar queda deshabilitado sin la clave; una clave incorrecta rechaza la extracción vía `verificar_clave_maestra`. Un retiro en ARS, o uno en USD por debajo del umbral, no pide clave maestra | H | 🔴 | ⬜ | |
| CAJ-39 | **"Efectivo USD" y "Caja Fuerte USD" se siembran automáticamente para todo tenant** (G5 Fase 5, F2, mig 373, 2026-08-19) | (a) Tenant nuevo (alta); (b) tenant existente (backfill de la migración) | Ambos casos: existe una `cuentas_origen` "Efectivo USD" (moneda USD) y una fila `cajas` "Caja Fuerte USD" (`es_caja_fuerte=true, moneda='USD'`), dormidas en $0 hasta que se usen — no requiere ningún alta manual para que la pestaña USD de la Bóveda tenga dónde acreditar dólares | M | 🟡 | ⬜ | |
| CAJ-40 | **🐛 Fix: auto-selección de caja al entrar a Caja no elegía la caja fuerte** (G5 Fase 5, hallazgo real durante el desarrollo, 2026-08-19) | Usuario sin caja preferida guardada entra al tab "Caja" con la sesión permanente de alguna Caja Fuerte (ARS o USD) abierta | `cajaSeleccionada` se autocompleta con una caja OPERATIVA abierta (nunca la Caja Fuerte) — antes, `cajasAbiertas[0]` podía devolver el `caja_id` de la Caja Fuerte si esa fila venía primero en la respuesta de la query (el comentario del código ya decía "nunca caja fuerte" pero el fallback no lo garantizaba realmente) | M | 🟡 | ⬜ | |
...
| FAC-28 | **NC emitidas RESTAN débito fiscal en Libro IVA / KPIs / liquidación / Posición IVA / dashboard** (HALLAZGO→FIX 2026-07-10, v1.125.0) | Emitir NC sobre venta facturada → ver Libro IVA Ventas, panel Facturación, Liquidación, Dashboard (overview + área Facturación) | La NC aparece como **fila negativa** en el Libro IVA Ventas (badge `NC-x #N`, neto e IVA en negativo; NC-C solo neto), el total del período la descuenta, los KPIs (débito/posición) y la liquidación 12m la restan **en el mes de su emisión** (`devoluciones.nc_fecha`, mig 266 — no el mes de la devolución). Export Excel incluye las filas NC | H | 🔴 | ✅ | *Fix 2026-07-10: `src/lib/libroIva.ts` + 11 unit (FAC-LIBRO-01→11) + e2e 86; antes las NC no restaban → débito sobre-declarado* |
| FAC-29 | **Guard de identidad de la EF `emitir-factura`** (HALLAZGO→FIX 2026-07-10, v1.125.0) | POST directo a la EF con el anon key pelado / con usuario de otro tenant | **401** "se requiere un usuario autenticado" con anon; **403** "no pertenece al tenant" con usuario ajeno; service_role pasa (flujos internos). Antes cualquiera con el anon key (público) podía emitir comprobantes de cualquier tenant conociendo venta_id+tenant_id | E | 🔴 | ✅ | *Fix 2026-07-10: guard antes de la lógica fiscal (index.ts §0); e2e 56 reescrito (401/403/400) verde en DEV* |
| FAC-30 | **Libros IVA por CUIT completo (sin filtro de sucursal)** (HALLAZGO→FIX 2026-07-10) | Con una sucursal seleccionada, abrir Libros IVA Ventas y Compras | Ambos libros muestran TODO el CUIT (el Libro Compras filtraba por sucursal y el de Ventas no → posición inconsistente). Nota visible: "Los libros IVA son del CUIT completo" | B | 🟡 | ✅ | *Fix 2026-07-10: quitado el filtro de sucursal de iva-compras; e2e 86 verifica la nota* |
...
| DSH-06 | **Reportes › Ventas: desglose por método de pago muestra el monto real en USD** (G5 Fase 7, H1, 2026-08-19) | Con ventas cobradas parcial o totalmente en "Efectivo USD" en el período, abrir Reportes → Ventas | "Total facturado" sigue siendo un único número en pesos (sin cambios); el chip del medio "Efectivo USD" en "Ingresos por método de pago" muestra AMBOS: el equivalente en pesos (como siempre) y, entre paréntesis, el monto real en dólares (`US$X`) — no ocultar el desglose | M | 🟡 | ⬜ | |
| DSH-07 | **Dashboard: ventas con componente USD excluidas del KPI/tendencia principal, mostradas aparte** (G5 Fase 7, H2, 2026-08-19) | Con ventas del mes que tengan `cotizacion_usd` no nulo, ver Dashboard → Insights | El total/tendencia de ventas del mes (comparación vs. mes anterior) NO incluye esas ventas — evita que una suba de cotización simule "más ventas" sin que el negocio haya vendido más. Aparece un insight aparte tipo "N ventas con componente USD este mes" con el equivalente en pesos, aclarando que están excluidas del total de arriba | H | 🟡 | ⬜ | |
| DSH-08 | **Rentabilidad: venta con componente USD no suma a los KPIs agregados pero sigue en "Detalle por venta"** (G5 Fase 7, H2, 2026-08-19) | Con una venta USD confirmada en el período, abrir Dashboard → Rentabilidad | "Ventas totales"/"Costo total"/"Ganancia bruta"/"Margen promedio" y el "Estado de resultados" NO incluyen esa venta (ni su costo). La venta SIGUE apareciendo en la tabla "Detalle por venta", tageada con un badge "USD" — no desaparece silenciosamente. El sub-texto de "Ventas totales" indica cuántas ventas USD quedaron afuera y su equivalente en pesos | M | 🟡 | ⬜ | |
| DSH-09 | **🐛 Ingreso Neto (Caja): Caja USD ya no se mezcla cruda dentro del total en pesos** (G5 Fase 7, H2 — bug preexistente corregido de paso, 2026-08-19) | Con movimientos reales en una sesión de Caja USD dentro del período del KPI "Ingreso Neto (Caja)" | El KPI en pesos solo suma movimientos `moneda='ARS'`; el monto USD real se muestra aparte en el sub-texto ("+US\$X"), nunca sumado como si fueran pesos. Antes del fix, un movimiento USD real (ej. "50") se sumaba directo al total en pesos sin convertir | H | 🔴 | ⬜ | |
...
| 12 | 🔢 **Serializados: SOPORTADOS (mig 313, v1.145.0)** | Se elige **a qué variante va cada serie** (no se reparte por cantidad): cada unidad es una serie concreta y su historial —garantía, RMA, recall— tiene que quedar bajo el SKU correcto. Las series **ya vendidas no se tocan**: quedan con el producto con el que se vendieron. En serializados nunca se re-apunta el LPN (nace uno nuevo), porque re-apuntarlo dejaría las series vendidas colgando de un LPN de otro producto | **e2e 112 (2º test)**: 3 series → 2 a 4G + 1 a 5G, conservación exacta, la vendida intacta, coherencia serie↔línea, par de movimientos neto cero |
| 12-bis | 🛑 **Rechazos server-side propios de series** | Serie ya vendida (es historial), la MISMA serie a dos variantes, serializado repartido "por cantidad" sin decir qué series, y destino que no maneja series igual que la madre. Ninguno mueve una sola serie | **e2e 112 (2º test)** + 5 negativos verificados en DEV |
...
## 🛑 §54 — El código desplegado de las Edge Functions fiscales 🛑 FISCAL — 2026-09-14
...
|---|---|---|---|
| 54.1 | La EF fiscal de PROD es la del repo | `bash scripts/auditar-edge-functions.sh emitir-factura` → `0` en DEV y PROD | ✅ 2026-09-14 |
| 54.2 | Doble click / reintento no emite dos CAE | e2e 21 y 42 contra DEV con la EF del repo (lock + cuarentena) | ✅ 12/12 y 45/45 |
...
| 66.2 | La conversión usa la misma tasa con la que el POS valúa el producto al cobrarlo (desde §70: vendedor divisa BNA del día hábil anterior; antes COMPRA) | unit `importarProductosMoneda.test.ts` + revisión de `VentasPage:1730` | ✅ |
...
| 66.6 | ✅ **RESUELTO (2026-09-25, mig 436)** — antes: `productos.margen_ganancia` era GENERATED `numeric(5,2)`, así que **ningún producto con más de 999,99 % de markup se podía guardar**, ni por CSV ni desde la ficha. Ahora `margen_ganancia` y `margen_objetivo` son `numeric(8,2)` (tope 999.999,99 %). Las demás `numeric(5,2)` (descuentos, comisiones) NO se tocaron: ahí 100 % es techo legítimo | ✅ **probado en DEV**: un producto con costo $30 y precio $1.500 guarda `margen_ganancia = 4900.00` y `margen_objetivo = 4900` (transacción descartada); unit del café a 4.900 % | ✅ DEV |
...
| 66.8 | ✅ **RESUELTO (2026-09-23)** — antes: el archivo de "Exportar productos" no era reimportable y el importador **pisaba la fila entera** con los valores por defecto. Medido en PROD: se perdían 19 proveedores, 19 descripciones, 12 códigos de barras, 9 productos con trazabilidad, y el IVA se reseteaba a 21. Ahora **al actualizar solo se escriben las columnas que el archivo trae** | unit `importarProductosActualizacion.test.ts` (23 casos) | ✅ |
| 66.9 | 🛑 `activo` estaba **fijo en `true`** en el payload: exportar y reimportar **reactivaba** los productos dados de baja, aunque el archivo exportado SÍ trae la columna. Es la prueba de que sumarle columnas al export no alcanzaba. Ahora solo se toca si el archivo la trae | unit | ✅ |
| 66.10 | Un archivo de `sku` + `precio` ya no exige la columna `nombre`: actualizar una lista de precios es el caso normal. Al **crear**, `nombre` sigue siendo obligatorio | unit + validación por fila | ✅ |
| 66.11 | 🛑 **REGLA #0** — si el producto está en dólares y el archivo trae el precio **sin** la columna de moneda, la fila se **rechaza**: el número es ambiguo (¿100 pesos o 100 dólares?) y asumir pesos lo dejaría a ~1/1400 de su valor. El costo y la venta se evalúan por separado | unit `precioAmbiguo` (5 casos) + consulta que trae `moneda_venta`/`moneda_costo` de los SKU existentes | ✅ |
| 66.12 | El precio y su moneda se escriben **en grupo**: cambiar la moneda sin recalcular el espejo en pesos dejaría el monto viejo con la moneda nueva | unit | ✅ |
| 66.13 | Una fila que no trae ninguna columna útil **no dispara UPDATE** | unit + guard en el envío | ✅ |
| 66.14 | Un archivo con `alicuota_iva = 0` (exento) **sí** actualiza el IVA: el 0 es un valor, no un vacío | unit `celdaTieneValor` | ✅ |
| 66.15 | ✅ **D-1 RESUELTO** — la ficha de producto calculaba el espejo en pesos con la cotización de **venta** mientras el POS cobra a la de **compra**. Era la mitad que quedó afuera del fix del 2026-09-08 (hallazgo de Fede: *"el POS convertía al dólar venta, así que le cobraba de más al cliente"*). Ahora **POS, ficha e importador usan la misma tasa** | revisión de `ProductoFormPage:66` vs `VentasPage:1730` vs `useCotizacion`; latente al arreglarlo (0 productos en USD en PROD) | ✅ |
| 66.16 | ✅ **D-2 mitigado en la ficha** — cargar un margen por encima del tope devolvía un `numeric field overflow` crudo de Postgres. Ahora avisa con el mismo mensaje que el importador, antes de intentar guardar. El tope pasó de 999,99 % a **999.999,99 %** (mig 436, ver 66.6) | validación en `handleSubmit` con `margenEntraEnLaBase` | ✅ |
| 66.17 | El export de productos emite las columnas de **producto** de la plantilla del importador, así se puede editar en Excel y volver a subir. Los booleanos salen como SI/NO. **No incluye las 14 columnas `estr_*`** (empaque, otra tabla): como una columna ausente significa "no tocar", reimportar no borra el empaque | revisión de `exportarProductos` | ✅ |
| 66.18 | 🛑 **El ida y vuelta no deforma los precios en dólares**: al exportar, un producto en USD sale con su **monto en dólares** y `USD` en la columna de moneda, no con el espejo en pesos — si no, cada reimportación lo multiplicaría por la cotización | unit `montoYMonedaParaExportar` + test de identidad exportar→importar sobre 3 productos | ✅ |
| 66.19 | 🛑🧾 **IVA Exento (0 %) se convertía en 21 % al importar** — `String(row.alicuota_iva \|\| '21')`: con 0, el `\|\|` devolvía '21' y la fila entraba SIN error, porque 21 es un valor válido. Un producto exento exportado y reimportado sin tocar nada facturaba IVA fantasma. Es el gotcha escrito en el CLAUDE.md, y ya estaba resuelto en `ProductoFormPage` con `Number.isFinite` — este archivo hermano nunca lo recibió | `celdaTieneValor` + `Number.isFinite`; unit del 0 como valor | ✅ |
| 66.20 | 🛑 **Traer la columna de moneda SIN el precio dejaba el precio en 0**, en silencio. El precio y su moneda se escriben en grupo y una columna ausente se parsea como 0, así que el CSV más natural ("le corrijo solo la moneda a este producto") zeroreaba un precio real. Ahora la fila se rechaza | unit `problemaDePrecio` — las dos direcciones, 9 casos | ✅ |
| 66.21 | `margen_objetivo = 0` sufría el mismo `\|\|`: se leía como vacío y la reimportación **borraba** el margen en vez de dejarlo en 0 | mismo patrón `celdaTieneValor` | ✅ |
| 66.22 | Una fila que trae SOLO columnas de empaque (`estr_*`) sobre un producto existente ya no falla entera: no hay UPDATE que hacer en `productos`, pero el empaque sí se aplica | guard `parcial vacío && !hasEstr` | ✅ |
| 66.23 | 🔒 El export de productos **ya no entrega costo ni margen objetivo a los roles que no los ven en la grilla** (CAJERO / DEPÓSITO / RRHH). Ocultar en la UI y no en la descarga no es ocultar nada | gate `verCosto` en `exportarProductos` | ✅ |
| 66.24 | El escape del CSV contempla saltos de línea: con `descripcion` y `notas` (texto libre) en el archivo, un Enter partía la fila en dos y corría las columnas al reimportar | regex `/[,"\n\r]/` | ✅ |
| 66.25 | 🛑 **El aviso de margen no saltaba en una actualización parcial** — con un CSV de `sku` + `precio_venta`, el costo ausente se leía como 0 y `margenGenerado(0, X)` da "entra siempre", así que un precio con un cero de más pasaba la vista previa y recién explotaba como `numeric field overflow` crudo al confirmar. Justo el CSV de una columna que este cambio promueve. Ahora el lado que el archivo no trae se toma **del valor real que ya está en la base** | consulta de existentes ampliada a `precio_costo`/`precio_venta` + comparación por lado | ✅ |
| 66.26 | Una fila con **cualquiera** de las 14 columnas `estr_*` aplica el empaque. Antes el predicado miraba solo 7, así que una fila con solo `estr_largo_unidad` fallaba entera | `hasEstr` se arma desde las columnas del archivo, no de una lista fija | ✅ |
| 66.27 | 🛑 **La red contra el CSV de monedas mezcladas sobrevive a la mig 436.** El techo de 999,99 % frenaba *de rebote* la fila con costo en ARS y precio en USD (150 ARS contra 100 USD a 1400 = 93.233 %). Al ampliar la columna esa fila habría entrado en silencio con el precio inflado por la cotización. Se conserva el mismo umbral, **solo** para filas con monedas distintas; en la misma moneda, un markup alto entra | unit `margenSospechosoPorMonedas` (5 casos) | ✅ |
| 66.28 | El **margen objetivo** de la ficha tenía `max="100"` en el input, así que el navegador no dejaba guardar un objetivo de 150 %, aunque es markup sobre el costo (`fn_precio_para_margen` = costo × (1 + m/100)) y no un porcentaje del precio. Ahora su tope es el de la columna, con aviso propio en `handleSubmit` | revisión de `ProductoFormPage` | ✅ código |

## 🔐 §67 — "Desactivar" un usuario le corta el acceso de verdad (mig 433) — 2026-09-24

Ambos modos. Salió contestando una pregunta de GO sobre cómo conviene manejar las cuentas de los empleados. 🛑 **Hasta acá, dar de baja a alguien NO le quitaba nada**: `get_user_tenant_id()` —la función que gobierna el `USING` de casi todas las policies de RLS— no miraba `activo`, ni lo hacía `users_select`, ni `loadUserData`, ni ningún guard del frontend. Y "Desactivar" es la única acción que existe sobre un usuario (no hay eliminar), así que desde la app **no había forma de cortarle el acceso a un empleado que se fue**.

| # | Escenario | Cómo se verifica | Estado |
|---|---|---|---|
| 67.1 | Un usuario dado de baja **no ve ningún dato** de su negocio: `get_user_tenant_id()` no resuelve para él, así que todas las policies lo rechazan | ✅ **probado en DEV** (2026-09-24, mig 433 aplicada) impersonando al supervisor del tenant de dev con `SET LOCAL ROLE authenticated` + `request.jwt.claims`: antes de la baja veía 26 productos / 4 ventas / 2 clientes / su propia fila; después, **0 / 0 / 0 / 0** y `get_user_tenant_id() = NULL` | ✅ |
| 67.2 | Un usuario dado de baja que todavía recuerda su contraseña entra, pero ve **"Tu acceso fue dado de baja"** con el botón de cerrar sesión — **NO** el formulario de crear un negocio | ✅ **spec e2e `158_acceso_revocado_mutante`**, con 2 usuarios reales: el DUEÑO lo da de baja desde /usuarios → el contador entra con su contraseña en un contexto limpio y ve la pantalla (y **no** `/onboarding`) → lo reactivan → vuelve a entrar normal | ✅ |
| 67.3 | 🛑 Sin el punto anterior, el usuario dado de baja caía en `needsOnboarding` y se le ofrecía **crear un negocio nuevo con su misma identidad**; el insert chocaba contra su fila vieja y moría con un error crudo de SQL. Lo marcó el `migration-reviewer` | revisión del `migration-reviewer` sobre `authStore`/`AuthGuard`/`LoginPage` | ✅ código |
| 67.4 | **`activo` es NULLABLE** (`boolean DEFAULT true`): todas las funciones usan `coalesce(activo, true)`, igual que `fn_soporte_ticket_detalle`. Con `AND activo` a secas, cualquier fila con NULL habría perdido el acceso | revisión del `migration-reviewer`, tabla de verdad completa | ✅ código |
| 67.5 | Nadie puede **darse de baja a sí mismo** ni dar de baja al **último DUEÑO activo** del negocio: antes era un error cosmético, ahora sería un candado sin llave | ✅ **probado en DEV**: los dos `UPDATE` rebotan con el mensaje del trigger (*"No podés darte de baja a vos mismo…"* / *"Es el único DUEÑO activo del negocio…"*). El primero saltó incluso sin buscarlo: el `UPDATE` de prueba corría con el `auth.uid()` del propio usuario | ✅ |
...
| 67.8 | 🛑 **Una baja por error tiene que poder deshacerse.** Hasta acá `UsuariosPage` gateaba TODAS las acciones con `canManage && u.activo`: para un usuario inactivo no se renderizaba ningún botón y no existía "Reactivar" en ningún lado. Cosmético antes de la 433; **candado sin llave** después. Se agregó el botón (mutación `reactivar`, solo DUEÑO) — la policy `users_update_owner` sí lo permite porque compara el `tenant_id` de la FILA, que la baja no toca | pasos 3 y 4 del spec e2e `158_acceso_revocado_mutante` | ✅ |
| 67.9 | El corte es **completo**: auditadas las 234 policies de `public`, las únicas que no pasan por `tenant_id`/`get_user_tenant_id()` son 3 de catálogo público sin datos del negocio (`planes`, `consumo_tarifas`, `ayuda_recursos` publicados). `get_user_role()` sigue sin mirar `activo`, pero toda policy que lo usa lo acompaña de `tenant_id = get_user_tenant_id()`, que ya da NULL | query sobre `pg_policies` en DEV, 2026-09-24 | ✅ |
| 67.10 | Reactivar vuelve a ocupar un lugar del plan: si el negocio está en el límite, `trg_enforce_usuarios` rechaza con *"Límite del plan alcanzado: usuarios…"* y el toast muestra ese mensaje, no uno genérico | 🟡 no probado — `Almacén Jorgito` tiene límite `-1` (ilimitado), así que el spec 158 no ejercita este camino | 🟡 |
...
## 🔑 §69 — Usuarios con nombre y contraseña, SIN correo (mig 434) — 2026-09-24

Pedido de GO: en un negocio chico los empleados no tienen mail propio, o tienen uno que no revisan nunca. Hasta acá `invite-user` exigía una dirección real (`inviteUserByEmail` manda un magic link), así que el dueño terminaba inventando casillas o usando la suya para todo el equipo.

**Las dos decisiones que tomó GO** (el resto salía de ahí): al ingresar el empleado escribe el **código del negocio + su usuario** —no una dirección—, y la contraseña que le pone el dueño es **de un solo uso**: está obligado a cambiarla en el primer ingreso. Por dentro la identidad de Auth es `<usuario>.<codigo>@u.genesis360.pro`, un dominio que no recibe correo; el empleado nunca la ve ni la escribe.

| # | Escenario | Cómo se verifica | Estado |
|---|---|---|---|
| 69.1 | El dueño crea al empleado desde Usuarios → "Sin email" con nombre, usuario y contraseña. **No se manda ningún mail** | spec e2e `159_usuario_sin_correo_mutante` (pasos 1); verificado en DB: `auth.users.email = e2e….almacenjorgito@u.genesis360.pro`, `users.usuario` cargado, `debe_cambiar_password = true` | ✅ |
| 69.2 | El empleado entra con **código del negocio + usuario + contraseña**, en una sesión que nunca vio la app | spec 159 (paso 2). El código lo muestra el propio formulario de alta, que es de donde el dueño lo saca para dictárselo | ✅ |
| 69.3 | Lo PRIMERO que ve es "Elegí tu contraseña", y no entra a ningún lado hasta cambiarla (el guard está en `AuthGuard`, no en una ruta, así que no se saltea por URL) | spec 159 (paso 3): la pantalla aparece y **no hay `aside`** — ni siquiera cargó el layout | ✅ |
| 69.4 | Con la contraseña NUEVA entra normal, y la VIEJA deja de servir | spec 159 (pasos 4 y 5) | ✅ |
| 69.5 | 🛑 **El dueño NO puede apagar la marca de "contraseña sin estrenar" desde el cliente.** `users_update_owner` le da UPDATE sin restricción de columna sobre las filas de su negocio: sin guard, podía bajar `debe_cambiar_password` con un PATCH directo a PostgREST, sin rotar nada, y seguir sabiendo la contraseña de su empleado para siempre — justo la garantía que la marca existe para dar. **Lo encontró el `migration-reviewer`; mi comentario en la migración afirmaba lo contrario y estaba mal** | trigger `trg_guard_debe_cambiar_password` (patrón de la mig 247). Probado en DEV con los 3 casos: el DUEÑO la apaga → **rebota**; el DUEÑO la prende (forzar cambio) → permitido; `service_role` la apaga → permitido | ✅ |
| 69.6 | 🛑 **Cambiar la contraseña con la Admin API revoca TODAS las sesiones del usuario, incluida la suya.** El empleado elegía su contraseña y la app lo escupía al login sin una palabra. **Lo encontró el spec e2e, no la revisión de código**: la bandera quedaba correctamente en `false` y la captura mostraba `/login`. Ahora la app lo vuelve a autenticar con la contraseña nueva —que tiene en la mano— y el reingreso es invisible | spec 159: tras el cambio aparece el toast Y el `aside` de la app | ✅ |
| 69.7 | El dueño solo puede reponer la contraseña de cuentas **sin correo**. Una cuenta con casilla real es de una persona que se autentica con su propia dirección (o con Google): dejar que el dueño del negocio se la pise sería quedarse con su cuenta, no administrar un empleado — esas se recuperan por "Olvidé mi contraseña", que les llega a ELLAS | guard en la EF (`if (!target.usuario)`), + la UI solo muestra el botón cuando `u.usuario`. 🟡 sin test | 🟡 |
| 69.8 | El código del negocio es **único e inmutable**: se genera del nombre en el alta y no se edita. Cambiarlo dejaría afuera a todos los usuarios sin correo del negocio, porque su dirección de Auth ya quedó fija | trigger `trg_tenant_codigo` + índice único. Verificado en DEV: los 9 negocios quedaron con código propio y sin colisiones (`almacenjorgito`, `familiaotranto`, …) | ✅ |
| 69.9 | 🛑 La dirección interna se compone con el `codigo` que sale de la BASE, nunca con uno que mande el cliente: si no, un dueño podría crear usuarios dentro del negocio de otro | la EF lee `tenants.codigo` por `tenant_id` del perfil del LLAMADOR, y el `tenant_id` tampoco se toma del body | ✅ código |
| 69.10 | El rol `ADMIN` no se puede asignar por esta vía (es rol de STAFF de Genesis360: `is_admin()` ve TODOS los negocios) | mismo whitelist que `invite-user`, + trigger de la mig 254 | ✅ código |
| 69.11 | Si el INSERT en `users` falla —el caso real es el **límite de usuarios del plan**— se borra la cuenta de `auth.users` recién creada. Sin esto queda un huérfano que se queda con el nombre para siempre: el dueño reintenta y le dice "ya existe" sin que aparezca en ningún lado | `deleteUser` en el catch de la EF. 🟡 sin test (Almacén Jorgito tiene límite -1) | 🟡 |
| 69.12 | El nombre de usuario es único **por negocio**, no global (decisión de GO: por eso se pide el código al ingresar). Dos negocios pueden tener cada uno su "juan" | índice único parcial `users_tenant_usuario_key` + chequeo con mensaje claro en la EF antes de crear | ✅ |
| 69.13 | Las reglas de formato del usuario viven en `src/lib/usuarioLocal.ts` **con tests** (16), y hay uno que verifica que todo lo que el front normaliza y da por válido pasa también el CHECK de la base — que es el que evitaría el error crudo de constraint | `tests/unit/usuarioLocal.test.ts`. Aplicación directa de la regla del §68 | ✅ |

**Lo que dejó esta tanda, para la próxima**: los dos agujeros 🛑 los encontraron dos cosas DISTINTAS, y ninguna era escribir el código con cuidado. El 69.5 lo vio una revisión independiente de la migración; el 69.6 no lo podía ver ninguna revisión de código —el código era correcto, el efecto estaba en GoTrue— y solo apareció cuando un test manejó la app como un usuario de verdad. Las dos cosas hacen falta.

## 💵 §70 — D-1 fase 2: UNA sola tasa USD→ARS, vendedor divisa BNA del día hábil anterior (mig 440) — 2026-09-26

Decisión de GO del 25/09 (D-1): todo lo que pasa dólares a pesos usa el **vendedor divisa del Banco Nación del día hábil anterior** (`fn_cotizacion_bna_vigente`, mig 439). Reemplaza la regla "USD→ARS a COMPRA" de v1.207.0 **y** el dólar "oficial" de dolarapi (que es BNA **billete**). Ya no hay carga manual del dólar (A-4: "automática si hay fuente"). Exposición en PROD al 26/09: 0 productos y 0 tiers en USD.

| # | Escenario | Cómo se verifica | Estado |
|---|---|---|---|
| 70.1 | El menú lateral muestra "Dólar BNA divisa $1.525,50 ARS · Vendedor · del 25/09" (la fecha SIEMPRE visible) y no deja cargar un valor a mano | sonda Playwright en DEV (captura del widget) | ✅ |
| 70.2 | Al entrar a la app se llama a la EF `cotizacion-bna`, que captura del BNA si la última captura tiene > 30 min (A-2, respaldo del cron) — montado en `AppLayout`, no solo en el widget (con el menú colapsado el widget no existe) | la sonda vio la llamada: `200 {ok:true, vigente:{fecha:2026-09-25, venta:1525.5}}` | ✅ |
| 70.3 | Si la captura falla, se sigue con la última y se avisa con su fecha; si la vigente tiene ≥ 5 días, aviso aunque nadie vea un error (fines de semana y feriados no llegan a tanto) | unit `cotizacionBna.test.ts` (`avisoCotizacion`) | ✅ |
| 70.4 | POS: un producto de USD 10 entra al carrito a 10 × tasa vigente | e2e `55_venta_usd_conversion_mutante` con el fixture aplicado (Coca Cola 1.5L → USD 10) y restaurado: **$15.255** | ✅ |
| 70.5 | 🛑 POS: un producto en USD **sin** cotización NO se vende al `precio_venta` en pesos guardado (congelado a la tasa del día en que se editó) — antes caía ahí en silencio. Ahora frena con aviso (D5) | e2e `160` (70.5): intercepta la EF y la RPC para quedar sin cotización (sin tocar la tabla global), anti-vacío = widget "Sin cotización". **Mutación verificada**: con el freno desactivado el test falla | ✅ |
| 70.6 | Precio, tiers, combos y pago recibido en USD usan la MISMA tasa → pagar USD 100 un producto de USD 100 no deja vuelto | unit `cotizacionBna.test.ts` + `cajaBoveda.test.ts` (BOV-CNV-08: ida y vuelta exacta) | ✅ |
| 70.7 | Bóveda: USD→$ y $→USD usan la misma tasa (antes compra y venta); el modal muestra "dólar BNA divisa X del dd/mm" | unit `cajaBoveda.test.ts` (8) | ✅ |
| 70.8 | 🛑 Pedidos → venta (`fn_pedido_generar_venta` → `fn_precio_venta_efectivo`): un producto en USD se cotizaba a `precio_venta` guardado (DEV: la pistola de USD 450 salía $695.250 = 450 × 1545 billete) y los tiers USD a `tenants.cotizacion_usd`. Ahora `precio_usd × tasa` = **$686.475**, igual que el POS; sin tasa, error | probado en DEV en transacción descartada (antes/después) + el caso sin cotización → `El producto tiene precio en dólares y no hay cotización del dólar BNA` | ✅ |
| 70.9 | La venta generada desde un pedido con un producto en USD queda sellada con `ventas.cotizacion_usd` (el POS ya lo hacía) → Dashboard y Rentabilidad la separan como venta con componente USD | e2e `160` (70.9): 2 × USD 10 → total = 2 × 10 × tasa (con `precio_venta` sembrado en $1 para detectar si vuelve a leer el espejo) y `cotizacion_usd` = tasa; control anti-vacío: pedido en pesos → `cotizacion_usd` NULL | ✅ |
...
| 71.5 | Solo se ofrecen categorías y proveedores **activos** | e2e `162` E: categoría desactivada ausente de "Listas"; la activa presente (anti-vacío) | ✅ |
...
| 72.7 | Producto **sin góndola** en modo prendido: no hay etiqueta que esperar → cambia a la hora | SQL en transacción descartada (E1-E4): modo prendido, producto sin góndola → aplicado a la hora, 0 tareas ligadas | ✅ |
