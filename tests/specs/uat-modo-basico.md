...
| CAJ-36 | **Bóveda bloquea Cajas USD como origen/destino, en 3 capas** (G5 Fase 3, hallazgo de code-review, 2026-08-18) | (a) Selectores de "Ingresar"/"Enviar" a Caja Fuerte con una Caja USD abierta; (b) modo básico con la Caja USD como caja activa; (c) CAJERO solicita transferencia de su Caja USD; (d) aprobar una solicitud vieja de una Caja USD | (a) el `<select>` filtra a solo cajas ARS; (b) en modo básico (origen fijo, sin selector) el botón "Depositar en Caja Fuerte" se oculta directamente en vez de fijar un origen inválido; (c) el botón "Caja Fuerte" (solicitud) no aparece para el CAJERO; (d) `aprobarSolicitudCajaFuerte` (NotificacionesButton.tsx) rechaza la aprobación igual, por si quedó una solicitud pendiente de antes. `operarCajaFuerte`/`enviarSolicitudFuerte` además tiran error si de algún modo llega un `sesion_id` en USD — la Bóveda todavía no tiene soporte multi-moneda (Fase 5) | B | 🟡 | ⬜ | |
