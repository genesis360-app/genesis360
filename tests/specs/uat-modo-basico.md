...
| 66.8 | 🛑 **Hallazgo aparte, PREEXISTENTE y más ancho que A0**: el archivo que genera "Exportar productos" **no es un archivo válido para reimportar**. El export emite solo `id, nombre, sku, precio_venta, precio_costo, stock_actual, stock_minimo, unidad_medida, activo, categoria`, y el importador **pisa todo el payload con los defaults** para lo que no viene. Medido en PROD (27 productos): se perderían **12** códigos de barras, **19** proveedores, **19** descripciones, **9** productos con trazabilidad (series/lote/vencimiento), 5 márgenes objetivo, 2 reglas de inventario y 1 kit. El IVA está a salvo hoy (**0** productos con alícuota ≠ 21) pero la misma vía lo resetearía a 21. A0 sumó la moneda a esa lista; no la creó | query de conteo en PROD + lectura del payload de `ImportarProductosPage` y de `exportarProductos` en `ProductosPage:731` | 🔴 abierto — avisado a GO |
