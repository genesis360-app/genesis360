...
| # | Escenario | Cómo se verifica | Estado |
|---|---|---|---|
| 67.1 | Un usuario dado de baja **no ve ningún dato** de su negocio: `get_user_tenant_id()` no resuelve para él, así que todas las policies lo rechazan | 🔴 **falta**: aplicar la mig 433 en DEV y probar con un usuario real dado de baja | 🔴 |
| 67.2 | Un usuario dado de baja que todavía recuerda su contraseña entra, pero ve **"Tu acceso fue dado de baja"** con el botón de cerrar sesión — **NO** el formulario de crear un negocio | 🔴 **falta la prueba manual**; el código está (`fn_estado_usuario_actual` + `accesoRevocado` + pantalla en `AuthGuard`) | 🔴 |
